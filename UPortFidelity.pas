...
type
  TFidelityData = class(TAdvancedForm)
    Panel2:           TPanel;
  ...
  private
    ...
    FSaleDateRow,FSalePriceRow:Integer;
    ...
    function GetCRM_BlackKnightsLPSData(sender: TObject):String;
    procedure LoadSubjectDataToSubjectGrid(Subject:TSubjectProperty);
    procedure SetupSubjectGridForBlackKnight(Subject:TSubjectProperty);
    procedure ClearSubjectGrid;
    procedure TransferFidelityDataToForm;
    procedure TransferBlackKnightDataToForm;
    procedure WriteCellValue(cellID:Integer;aValue:String);


  public
    FLPSSourceID:Integer;
...
procedure RequestFidelityData(doc: TContainer;cmd:Integer);
procedure RequestBlackKnightLPSData(doc: TContainer;cmd:Integer);

var
  FidelityData: TFidelityData;
...
// - UServiceManager.CheckAvailability(stFloodMaps);
  ...
  FidelityData := TFidelityData.Create(doc);
  try
    try
      FidelityData.FLPSSourceID := cmd;
      FidelityData.ShowModal;
    except
...
procedure RequestBlackKnightLPSData(doc: TContainer; cmd:Integer);
var
  SubjectLPS: TFidelityData;
begin
  if assigned(doc) then
    doc.SaveCurCell;         //save changes
  SubjectLPS := TFidelityData.Create(doc);
  try
    try
      SubjectLPS.FLPSSourceID := cmd;
      SubjectLPS.Caption := 'Black Knights Property Data Service';
      SubjectLPS.AnimateProgress.Visible := False;
      SubjectLPS.chkFidelityTransferSubjectDataSource.Caption := 'Transfer "Black Knights" as the Data Source';
      SubjectLPS.chkFidelityTransferSubject.Caption := 'Transfer "Black Knights" as the Data Source for Prior Sales';
      SubjectLPS.chkFidelityTransferSubjectVerificationSource.Caption := 'Transfer "Black Knights" as the Verification Source';
      SubjectLPS.Panel5.Visible := False;
      SubjectLPS.chkOverrideTextSubject.Checked := True;
...
constructor TFidelityData.Create(AOwner: TComponent);
var
  GlobePath: string;
begin
  ...
  FHasAnimateFile := not (FLPSSourceID = cmdLPSBlackKnightData);  //disable the annimation progress for BlackKnights
...
destructor TFidelityData.Destroy;
begin
...
procedure TFidelityData.LoadFieldNames;
var
  i, j, n: integer;
begin
...
procedure TFidelityData.btnLocateClick(Sender: TObject);
begin
  case FLPSSourceID of
          cmdFidelityData: GetCFDB_FidelityInfo(sender);
    cmdLPSBlackKnightData: GetCRM_BlackKnightsLPSData(sender);
  end;
end;

function TFidelityData.GetCFDB_FidelityInfo(Sender: TObject): String;
var
  i, n, j:  integer;
  IMessage: integer;
  SMessage: WideString;
...
function TFidelityData.GetCRM_BlackKnightsLPSData(Sender: TObject): String;
//var
//  Subject:TSubjectProperty;
begin
  Application.ProcessMessages;
  try
    if FPropInfo.SubjectProperty = nil then
      FPropInfo.SubjectProperty := TSubjectProperty.Create;
    if FDataLoaded and (tsSubjectGrid.Rows > 0) then
      begin
        if CompareText(edtAddress.Text, FPropInfo.SubjectProperty.Address) = 0 then
          begin
            if not OK2Continue('You''ve already pulled in public record for this property.  Would you like to get public record again?') then
              exit;
          end;
      end;
    ...
    if not FDataLoaded then
      begin
        ...
        begin
          //LoadLPSSubjectToPropInfo(FDoc,'',FPropInfo);   //for testing only
          TabSubjectResults.TabVisible := true;
          RzPageControl1.ActivePage := TabSubjectResults;
//        Subject := FPropInfo.SubjectProperty;
          LoadSubjectDataToSubjectGrid(FPropInfo.SubjectProperty);
        end;
      end;
  finally
  end;
...
procedure TFidelityData.ClearSubjectGrid;
var
  i: Integer;
begin
  with tsSubjectGrid do //clear any previous data
  begin
    ...
    begin
      Cell[1, i] := '';
      Cell[2, i] := '';
      CellColor[2,i] := clWhite;
    end;
  end;
end;

procedure TFidelityData.SetupSubjectGridForBlackKnight(Subject:TSubjectProperty);
var
  aRow:Integer;
begin
  ClearSubjectGrid;

  tsSubjectGrid.Rows := rFirePlace;
  tsSubjectGrid.Col[cName].Width := 200;
  tsSubjectGrid.Col[cValue].Width := 300;
  tsSubjectGrid.Col[cName].Color := rgb(255, 255, 217);
  tsSubjectGrid.Col[cValue].Color := clWindow;

  tsSubjectGrid.Cell[cName,rAddress]  := 'Address';
  tsSubjectGrid.Cell[cName,rCity]     := 'City';
  tsSubjectGrid.Cell[cName,rState]    := 'State';
  tsSubjectGrid.Cell[cName,rZip]      := 'Zip';
  tsSubjectGrid.Cell[cName,rCounty]   := 'County';
  tsSubjectGrid.Cell[cName,rAPN]          := 'APN';
  tsSubjectGrid.Cell[cName,rCensusTr]     := 'Census Tract';
  tsSubjectGrid.Cell[cName,rOwner]        := 'Owner Name';
  tsSubjectGrid.Cell[cName,rLandValue]    := 'Assessed Land Value';
  tsSubjectGrid.Cell[cName,rImprovement]  := 'Assessed Improvement';
  tsSubjectGrid.Cell[cName,rTotalAssess]  := 'Total Assessed Value';
  tsSubjectGrid.Cell[cName,rTaxYear] := 'Tax Year';
  tsSubjectGrid.Cell[cName,rTaxAmt]  := 'Tax Amount';
  tsSubjectGrid.Cell[cName,rLegalDesc]  := 'Legal Brief Description';
  tsSubjectGrid.Cell[cName,rSiteArea]   := 'Site Area';
  tsSubjectGrid.Cell[cName,rStories]    := 'Stories';
  tsSubjectGrid.Cell[cName,rDesignStyle] := 'Design Style';
  tsSubjectGrid.Cell[cName,rGLA]        := 'GLA';
  tsSubjectGrid.Cell[cName,rLatitude]   := 'Latitude';
  tsSubjectGrid.Cell[cName,rLongitude]  := 'Longitude';
  tsSubjectGrid.Cell[cName,rYearBuilt]  := 'Year Built';
  tsSubjectGrid.Cell[cName,rBedrooms]  := 'Bedrooms';
  tsSubjectGrid.Cell[cName,rBaths]  := 'Baths';
  tsSubjectGrid.Cell[cName,rTotalRooms]  := 'Total Rooms';
  tsSubjectGrid.Cell[cName,rBsmt] := 'Basement';
  tsSubjectGrid.Cell[cName,rBsmtFin]  := 'Basement Finished';
  tsSubjectGrid.Cell[cName,rGarage]  := 'Garage Num Cars';
  tsSubjectGrid.Cell[cName,rPool]  := 'Pools';
  tsSubjectGrid.Cell[cName,rFirePlace]  := 'FirePlace';
end;

procedure TFidelityData.LoadSubjectDataToSubjectGrid(Subject:TSubjectProperty);
var
  aRow,i:Integer;
  zipCode:String;
...
begin
  ...
  try
    ...
    aFont.Color := clNavy;
    aFont.Name := 'Segoe UI';
    tsSubjectGrid.Col[cName].Font := aFont;

    SetupSubjectGridForBlackKnight(Subject);
    aRow:=1;  //start with row 1
    tsSubjectGrid.RowColor[aRow] := rgb(217,217,255);

    tsSubjectGrid.Cell[cValue,rAddress]  := CamelCapStr(Subject.Address);
    tsSubjectGrid.Cell[cValue,rCity]  := CamelCapStr(Subject.City);
    tsSubjectGrid.Cell[cValue,rState] := Subject.State;
    if length(Subject.Zip4) > 0 then
      zipCode := Subject.Zip + '-' +Subject.Zip4
    else
      zipCode := Subject.Zip;
    tsSubjectGrid.Cell[cValue,rZip] := ZipCode;
    tsSubjectGrid.Cell[cValue,rCounty] := Subject.County;
    tsSubjectGrid.Cell[cValue,rAPN] := Subject.APN;

    tsSubjectGrid.Cell[cValue,rCensusTr] := Subject.CensusTract;
    tsSubjectGrid.Cell[cValue,rOwner] := Subject.Owner;
    tsSubjectGrid.Cell[cValue,rLandValue] := FormatFloat('$#,###',Subject.AssessedLandValue);
    tsSubjectGrid.Cell[cValue,rImprovement] := FormatFloat('$#,###',Subject.AssessedImproveValue);
    tsSubjectGrid.Cell[cValue,rTotalAssess] := FormatFloat('$#,###',Subject.TotalAssessedValue);
    tsSubjectGrid.Cell[cValue,rTaxAmt] := FormatFloat('$#,###',Subject.Taxes);
    tsSubjectGrid.Cell[cValue,rLegalDesc] := Subject.BriefLegalDescription;
    tsSubjectGrid.Cell[cValue,rGLA] := Format('%d',[Subject.GLA]);
    tsSubjectGrid.Cell[cValue,rSiteArea] := FormatFloat('#,### sqft',Subject.SiteArea);
    tsSubjectGrid.Cell[cValue,rStories]  := Format('%d',[Subject.Stories]);
    tsSubjectGrid.Cell[cValue,rDesignStyle] := Subject.DesignStyle;
    tsSubjectGrid.Cell[cValue,rLatitude] := FormatFloat('####.##########',Subject.Latitude);
    tsSubjectGrid.Cell[cValue,rLongitude] := FormatFloat('####.##########',Subject.Longitude);
    tsSubjectGrid.Cell[cValue,rYearBuilt] := Format('%d',[Subject.YearBuilt]);
    tsSubjectGrid.Cell[cValue,rBedrooms]  := Format('%d',[Subject.Bedrooms]);
    tsSubjectGrid.Cell[cValue,rBaths] := Format('%.1f',[Subject.Bath]);
    tsSubjectGrid.Cell[cValue,rTotalRooms] := Format('%d',[Subject.TotalRooms]);
    tsSubjectGrid.Cell[cValue,rGarage] := Format('%d',[Subject.Garage]);
    tsSubjectGrid.Cell[cValue,rPool] := Format('%d',[Subject.Pool]);
    tsSubjectGrid.Cell[cValue,rFirePlace] := Format('%d',[Subject.FirePlace]);
    tsSubjectGrid.Cell[cValue,rTaxYear] := Format('%d',[subject.TaxYear]);
    aRow := rFirePlace;
    //This is for Sales History
    if Subject.FSalesHistory <> nil then
      begin
        for i:= 0 to Subject.FSalesHistory.Count -1 do
          begin
            SalesHistory := TSalesHistory.Create;
            try
              if Subject.FSalesHistory.Count > 1 then
                tsSubjectGrid.Rows := aRow + ((Subject.FSalesHistory.Count-1) * 8) //total # of subject grid rows
              else
                 tsSubjectGrid.Rows := aRow + (Subject.FSalesHistory.Count * 8); 
              SalesHistory := TSalesHistory(Subject.FSalesHistory[i]);
              if SalesHistory = nil then continue;
              tsSubjectGrid.Cell[cName,aRow]   := '';
              tsSubjectGrid.CellColor[cValue,aRow] := rgb(202, 221, 255);
              tsSubjectGrid.CellColor[cName,aRow] := rgb(202, 221, 255);
              tsSubjectGrid.CellFont[cValue,aRow] := aFont;
              tsSubjectGrid.Cell[cValue,aRow]  := Format('Sales History Record #%d',[i+1]);  inc(aRow);
              tsSubjectGrid.Cell[cName,aRow] := Format('Sales History #%d',[i+1]);
              tsSubjectGrid.Cell[cName,aRow] := 'Buyer Name #1';
              tsSubjectGrid.Cell[cValue,aRow] := Format('%s %s',
                                                 [SalesHistory.Buyer1FirstName,
                                                  SalesHistory.Buyer1LastName]); inc(aRow);

              tsSubjectGrid.Cell[cName,aRow] := 'Buyer Name #2';
              tsSubjectGrid.Cell[cValue,aRow] := Format('%s %s',
                                                 [SalesHistory.Buyer2FirstName,
                                                  SalesHistory.Buyer2LastName]); inc(aRow);

              tsSubjectGrid.Cell[cName,aRow] := 'Seller Name #1';
              tsSubjectGrid.Cell[cValue,aRow] := Format('%s %s',
                                                 [SalesHistory.Seller1FirstName,
                                                  SalesHistory.Seller1LastName]); inc(aRow);
              tsSubjectGrid.Cell[cName,aRow] := 'Seller Name #2';
              tsSubjectGrid.Cell[cValue,aRow] := Format('%s %s',
                                                 [SalesHistory.Seller2FirstName,
                                                  SalesHistory.Seller2LastName]); inc(aRow);

              tsSubjectGrid.Cell[cName,aRow]  := 'Deed Type';
              tsSubjectGrid.Cell[cValue,aRow] := SalesHistory.DeedType;  inc(aRow);
              if i = 0 then
                begin
                  FSaleDateRow   := aRow;
                  FSalePriceRow  := aRow+1;
                end;
              tsSubjectGrid.Cell[cName,aRow]  := 'Sale Date';
              tsSubjectGrid.Cell[cValue,aRow] := SalesHistory.SaleDate; inc(aRow);

              tsSubjectGrid.Cell[cName,aRow]  := 'Sale Price';
              tsSubjectGrid.Cell[cValue,aRow] := FormatFloat('$#,###',SalesHistory.SalePrice);  inc(aRow);


              tsSubjectGrid.Cell[cName,aRow]  := 'REO';
              tsSubjectGrid.Cell[cValue,aRow] := Format('%d',[SalesHistory.REOFlag]);
...
function TFidelityData.VerifyRequest: boolean;
begin
...
procedure TFidelityData.btnSaveFidelityPrefsClick(Sender: TObject);
begin
...
procedure TFidelityData.tsCompGridComboRollUp(Sender: TObject; Combo: TtsComboGrid; DataCol, DataRow: integer);
var
  FormUID: TFormUID;
  i, n:    integer;
begin
...
procedure TFidelityData.LoadComboGrid;
var
  i, j: integer;
begin
  if assigned(FDocCompTable) then    //free existing object, if applicable
...
procedure TFidelityData.btnTransferSubjectClick(Sender: TObject);
begin
  case FLPSSourceID of
    cmdFidelityData: TransferFidelityDataToForm;
    cmdLPSBlackKnightData: TransferBlackKnightDataToForm;
  end;
end;

procedure TFidelityData.WriteCellValue(cellID:Integer;aValue:String);
var
  valueExists:Boolean;
  aCellText:String;
begin
  aCellText := FDoc.GetCellTextByID(cellID);
  ValueExists := length(aCellText) > 0;
  if chkOverrideTextSubject.checked or not ValueExists then  //only set the cell value if OVERWRITE ON or cell is EMPTY
    FDoc.SetCellTextByID(cellID, aValue);
end;

procedure TFidelityData.TransferBlackKnightDataToForm;
const
  Black_Knights = 'Black Knights';
var
  aLatLon: String;
  BsmtFinishPct,Bsmt,BsmtFin: Integer;
begin
  WriteCellValue(46,tsSubjectGrid.Cell[cValue,rAddress]);
  WriteCellValue(47,tsSubjectGrid.Cell[cValue,rCity]);
  WriteCellValue(48,tsSubjectGrid.Cell[cValue,rState]);
  WriteCellValue(49,tsSubjectGrid.Cell[cValue,rZip]);
  WriteCellValue(50,tsSubjectGrid.Cell[cValue,rCounty]);
  WriteCellValue(148, tsSubjectGrid.Cell[cValue,rStories]);
  WriteCellValue(149,tsSubjectGrid.Cell[cValue,rDesignStyle]);
  WriteCellValue(59, tsSubjectGrid.Cell[cValue,rLegalDesc]);
  WriteCellValue(60, tsSubjectGrid.Cell[cValue,rAPN]);
  WriteCellValue(367,tsSubjectGrid.Cell[cValue,rTaxYear]);
  WriteCellValue(368,tsSubjectGrid.Cell[cValue,rTaxAmt]);
  WriteCellValue(599, tsSubjectGrid.Cell[cValue,rCensusTr]);
  WriteCellValue(58, tsSubjectGrid.Cell[cValue,rOwner]);
  WriteCellValue(232, tsSubjectGrid.Cell[cValue,rGLA]);
  WriteCellValue(229, tsSubjectGrid.Cell[cValue,rtotalRooms]);
  WriteCellValue(230,tsSubjectGrid.Cell[cValue,rBedrooms]);
  WriteCellValue(231, tsSubjectGrid.Cell[cValue,rBaths]);
  WriteCellValue(67, tsSubjectGrid.Cell[cValue,rSiteArea]);
  WriteCellValue(2030,tsSubjectGrid.Cell[cValue,rGarage]);
  aLatLon := Format('%s;%s',[tsSubjectGrid.Cell[cValue,rLongitude],tsSubjectGrid.Cell[cValue,rLatitude]]);
  WriteCellValue(9250, aLatLon);
  WriteCellValue(200,tsSubjectGrid.Cell[cValue,rBsmt]);
  Bsmt := getValidInteger(tsSubjectGrid.Cell[cValue,rBsmt]);
  BsmtFin := getValidInteger(tsSubjectGrid.Cell[cValue,rBsmtFin]);
  if Bsmt > 0 then
    begin
      BsmtFinishPct := round((BsmtFin/Bsmt) *100);
      FDoc.SetCellTextByID(201, Format('%d',[BsmtFinishPct]));
    end;
  //Populate sales history load the most reason to subject column history
  if chkRecordingDateContract.Checked then
    WriteCellValue(2053, tsSubjectGrid.Cell[cValue, FSaleDateRow]);
  if chkRecordingDatePrior.Checked then
    WriteCellValue(934,tsSubjectGrid.Cell[cValue, FSaleDateRow]);
  if chkSalesPricePrior.Checked then
    WriteCellValue(935,tsSubjectGrid.Cell[cValue, FSalePriceRow]);
  WriteCellValue(2074,FormatDateTime('mm/dd/yyyy',now));
  if chkRecordingDateSubjectGrid.Checked then
    WriteCellValue(960, tsSubjectGrid.Cell[cValue, FSaleDateRow]);
  WriteCellValue(936,Black_Knights);

    //preferences
    if chkAssessedLand.Checked then
      WriteCellValue(870, tsSubjectGrid.Cell[cValue, rLandValue]);  //land('site') value for cost approach
    if chkAssessedImprovements.Checked then
      WriteCellValue(871, tsSubjectGrid.Cell[cValue, rImprovement]);  //'site' improvements for cost approach
    if chkFidelityTransferSubject.Checked then
    begin
      WriteCellValue(936, Black_Knights);
      WriteCellValue(2074, DateToStr(Today));
    end;
    if chkAPNTransferSubj.Checked then
      WriteCellValue(926, FDoc.GetCellTextByID(60));
    if chkSalesPriceContract.Checked then
      WriteCellValue(2052, tsSubjectGrid.Cell[cValue, FSalePriceRow]);
    if not chkSalesPriceGrid.Checked then
      WriteCellValue(947, '');
    if chkFidelityTransferSubjectVerificationSource.Checked then
      WriteCellValue(931, Black_Knights);
    if chkFidelityTransferSubjectDataSource.Checked then
      WriteCellValue(930, Black_Knights);

  if Ok2Continue('Subject property data is transferring successfully.  Would you like to close this dialog now?') then
    btnClose.Click;
end;

procedure TFidelityData.TransferFidelityDataToForm;
var
  j, k:      integer;
  F:         Textfile;
  S, existText: string;
...
procedure TFidelityData.btnTransferCompsClick(Sender: TObject);
var
  i, j, k, m: integer;
  F:          Textfile;
  S, existText: string;
...
procedure TFidelityData.btnInsertCompsPageClick(Sender: TObject);
var
  FormUID: TFormUID;
  n:       integer;
begin
...
procedure TFidelityData.btnCloseClick(Sender: TObject);
begin
  Close;  //goes to FormCloseQuery
  //CheckServiceExpiration;
end;

procedure TFidelityData.FormCloseQuery(Sender: TObject; var CanClose: boolean);
begin
  CanClose := true;
  ...
  begin
    if (FHasSelectedComp = false) and (FLPSSourceID = cmdFidelityData) then
...
procedure TFidelityData.TransferDueDiligence;  //transfer search results to Due Diligence form
var
  form:    TDocForm;
  FormUID: TFormUID;
begin
...
procedure TFidelityData.FormShow(Sender: TObject);
...
begin
  ...
  //subject result tab
  ChkBoxTopAdj := Round((lblOverrideTextSubject.Height - lblOverrideTextSubject.Height) / 2);
  chkOverrideTextSubject.Top := lblOverrideTextSubject.Top + ChkBoxTopAdj;
  lblOverrideTextSubject.Font.Size := Font.Size;
  btnTransferSubject.Left := lblOverrideTextSubject.Left + lblOverrideTextSubject.Width + 10;
end;
