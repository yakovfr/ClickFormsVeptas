...
'PCVAC-31: Both boxes are checked, for Units (One, One w/Acc Unit).  Check one box.
'PCVAC-32: Neither box is checked, for Units (One, One w/Acc Unit).  Check one box.
...
'PCVAC-45: Basement type, no box checked.  Check box(es) that apply.
...
'PCVAC-31: Both boxes are checked, for Units (One, One w/Acc Unit).  Check one box.
PCVAC31Msg = "** Both boxes are checked, for Units (One, One w/Acc Unit).  Check one box."
if isChecked(1, 136) and isChecked(1, 137) then
  AddRec PCVAC31Msg, 1, 136
end if


'PCVAC-32: Neither box is checked, for Units (One, One w/Acc Unit).  Check one box.
PCVAC32Msg = "** Neither box is checked, for Units (One, One w/Acc Unit).  Check one box."
if not isChecked(1, 136) and not isChecked(1, 137) then
  AddRec PCVAC32Msg, 1, 136
end if
...
if GetValue(1, 169) > 0 then
  if not isChecked(1, 168) then
    AddRec PCVAC34Msg, 1, 168
  end if
end if
...
if GetValue(1, 171) > 0 then
  if not isChecked(1, 170) then
    AddRec PCVAC35Msg, 1, 170
  end if 
end if
...
if isChecked(1, 180) then
  if GetText(1, 181) = "" then
    AddRec PCVAC36Msg, 1, 181
  end if
end if
...
if isChecked(1, 170) then
  if GetValue(1, 171) = 0 then
    AddRec PCVAC37Msg, 1, 171
  end if 
end if


'PCVAC-45: Basement type, no box checked.  Check box(es) that apply.
PCVAC45Msg = "** Basement type, no box checked.  Check box(es) that apply"
chkCount = GetCheck(1, 140) + GetCheck(1, 141) + GetCheck(1, 142) + GetCheck(1, 143)
if chkCount = 0 then
  AddRec PCVAC45Msg, 1, 140
end if
