'FM00364_PCV.vbs 1025 - 2000A Extra Comps Reviewer Script
...
isMatch = False
If (GetText(1, 65)<> "") and (strComp(GetText(1,35), GetText(1,95)) = 0) Then   
   isMatch = True
End If
If (GetText(1, 148)<>"") and (strComp(GetText(1,35), GetText(1,178)) = 0) Then   
   isMatch = True
End If
If (GetText(1, 231)<>"") and (strComp(GetText(1,35), GetText(1,261)) = 0) Then   
   isMatch = True
End If
If not isMatch Then
   AddRec  GSU_AG_QUAL, 1, 35
ENd If
...
'PCV #25b: Make sure Subject condition is matched or bracketed by comparable condition.
GSU_AG_QUAL = "**THE SUBJECT'S CONDITION IS NOT MATCHED BY THE COMPARABLES PRESENTED."
isMatch = False
If (GetText(1, 65)<> "") and (strComp(GetText(1,37), GetText(1,99)) = 0) Then   
   isMatch = True
End If
If (GetText(1, 148)<>"") and (strComp(GetText(1,37), GetText(1,182)) = 0) Then   
   isMatch = True
End If
If (GetText(1, 231)<>"") and (strComp(GetText(1,37), GetText(1,265)) = 0) Then   
   isMatch = True
End If
If not isMatch Then
   AddRec  GSU_AG_QUAL, 1, 37
ENd If

   Require IDSUBGBA2, 1, 38
'PCV #26: Make sure subject GLA is bracketed by  comp. gla.
gSub = GetValue(1,38)
gComp1 = GetValue(1,101)
gComp2 = GetValue(1,184)
gComp3 = GetValue(1,267)
