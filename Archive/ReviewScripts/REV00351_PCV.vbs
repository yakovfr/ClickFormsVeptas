...
if isChecked(2, 37) then
  if GetValue(2, 38) = 0 then
    AddRec PCVAC33Msg, 2, 38
  end if 	
end if
...
if GetValue(2, 38) > 0 then
  if not isChecked(2, 37) then
    AddRec PCVAC34Msg, 2, 37
  end if
end if

'PCVAC-35: Number of Woodstove(s) is more than "0", but Woodstove(s) box is not checked.  Correction required
PCVAC35Msg = "** Number of Woodstove(s) is more than '0', but Woodstove(s) box is not checked.  Correction required"
if GetValue(2, 40) > 0 then
  if not isChecked(2, 39) then
     AddRec PCVAC35Msg, 2, 40
  end if 
end if
...
if isChecked(2, 45) then
  if GetText(2, 46) = "" then
    AddRec PCVAC36Msg, 2, 46
  end if
end if
