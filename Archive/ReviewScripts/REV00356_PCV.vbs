'REV00356_PCV.vbs 2055 Reviewer Script
' PCV rules:
...
' 12/17/2014: add new rule #22: lender name must be PCV Murcor
' 04/17/2013: update to check effective date is AFTER signature date give hard stop
...
Dim NumLicenses
  ...
  If hasText(3, 27) Then
    ...
    OnlyOneCheckOfTwo 3, 38, 39, IDCERTSUBINSPECT, "SUP APPRAISER INSP SUBJECT: Both Did and Did Not are checked"
    If isChecked(3, 39) Then
      Require IDCERTINSPECTEXTDATE, 3, 40
    End If
    If GetCellUADError(3, 40) Then
      AddRec IDCERTINSPECTEXTDATEUADERROR, 3, 40
    End If

    OnlyOneCheckOfTwo 3, 41, 42, IDCERTCOMPINSPECT, "SUP APPRAISER INSP COMPS: Both Did and Did Not are checked"
    If isChecked(3, 42) Then
      Require IDCERTDATEOFINSPECTCOMP, 3, 43
    End If
    If GetCellUADError(3, 43) Then
      AddRec IDCERTDATEOFINSPECTCOMPUADERROR, 3, 43
    End If
  End If
