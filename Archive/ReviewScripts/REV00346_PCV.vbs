...
'FM00346.vbs 1073 Reviewer Script
' 04/17/2013: Update if Effective date is AFTER than signature date give hard stop.
...
'09/23/2014 Fixed bug: check is date valid (isDate) before use DateDiff
...
Dim NumLicenses
  ...
  AddTitle String(5, "=") + "FNMA 1073 Cert " + IDAPPCERT + String(5, "=")
  ...
  'PCV #1: -- DateDiff will return # of days that are different between the 2 dates
  lDate  = GetText(3,18)
  eDate = GetText(3,12)
  RECAPPCERTLICEXPDAT = "**EFFECTIVE DATE IS AFTER EXPIRATION DATE.  PLEASE MODIFY."
  if isDate(ldate) and isDate(eDate) then
  ...
  If hasText(3, 27) Then
    ...
    OnlyOneCheckOfThree 3, 38, 39, 41, IDCERTSUBINSPECT, "SUP APPRAISER SUB INSPECT: Multiple Items are Checked"
    ...
    End If
    OnlyOneCheckOfTwo 3, 43, 44, IDCERTCOMPINSPECT, "SUP APPRAISER COMP INSPECT: Both Did and Did Not are Checked"
