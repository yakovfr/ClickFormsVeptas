'REV00347_PCV.vbs Exterior Condo Reviewer Script
' 05/11/2015: update Freddie Mac rules
'RULE # FRE1093: Condominium Zoning - Triggers if the Site Zoning Compliance Type is Legal Nonconforming and the Site Zoning Permit Rebuild to Current Density Indicator is "No"
'PCVAC-33: Fireplace(s) box is checked but # of Fireplace(s) is not more than "0".  Provide number of Fireplaces.
'PCVAC-34: Number of Fireplace(s) is more than "0", but Fireplace(s) box is not checked.  Correction required
'PCVAC-35: Number of Woodstove(s) is more than "0", but Woodstove(s) box is not checked.  Correction required
'PCVAC-36: Other box is checked but description field is blank.  Comment required, describe other amenity.
'PCVAC-37: Woodstove(s) box is checked but # of woodstoves is not more than "0".  Provide number of woodstoves.
'PCVAC-47: Number of Bath(s) field is "N/A".  Provide number of baths.
'PCVAC-48: Number of Bath(s) field is blank.  Provide number of baths.
'PCVAC-49: Number of Bedrooms field is "N/A".  Provide number of bedrooms.
'PCVAC-50: Number of Bedrooms field is blank.  Provide number of bedrooms.


' 04/08/2014: Add more rules:
' Only do the check of date/price sale/transfer for the subject if subject of my research is checked
' Do the same check of date/price sale/transfer for the comp if comp of my research is checked
' Only warn if tax amount > 0 and per year/per month check box is not checked.
' 03/11/2014: Fix error: Subject Car Storage quantity mismatch, it points to the wrong cell: should be 2, 79 not 2, 76
' Fix ReviewContract section to get rid of the extra '(' when calling hasText function.
'
Sub ReviewForm
  ReviewSubject
  AddTitle ""
  ReviewContract
  AddTitle ""
  ReviewNeighborhood
  AddTitle ""
  ReviewSite
  AddTitle ""
  ReviewImprovements
  AddTitle ""
  ReviewProjectAnalysis
  AddTitle ""
  ReviewSubjectUnit
  AddTitle ""
  ReviewPriorSales
  AddTitle ""
  ReviewSalesSubject
  AddTitle ""
  ReviewIncomeApproach
  AddTitle ""
  ReviewReconciliation
  AddTitle ""
End Sub


Sub ReviewSubject
   
  AddTitle String(5, "=") + "FNMA 1075 " + IDSUBJECT + String(5, "=")

  Require IDFILENUM, 1, 3
  Require IDADDRESS, 1, 8
  If IsUAD Then
    Require IDUNITS_UAD, 1, 9
  Else
    Require IDUNITS, 1, 9
  End IF
  Require IDCity, 1, 10
  Require IDState, 1, 11
  Require IDZip, 1, 12
  Require IDBorrower, 1, 13
  Require IDOWNERPUB, 1, 14
  Require IDCounty, 1, 15
  Require IDLegal, 1, 16
  Require IDAPN2, 1, 17
  Require IDTaxYear, 1, 18
  If GetCellUADError(1, 18) Then
    AddRec IDTAXYEARUADERROR, 1, 18
  End If
  Require IDReTaxes, 1, 19
  Require IDProjName2, 1, 20
  Require IDPhase, 1, 21
  Require IDMapref, 1, 22
  Require IDCensus, 1, 23
  OnlyOneCheckOfThree 1, 24, 25, 26, IDOccNoCheck, IDOccChk
  Require IDSpecial, 1, 27
   
  if not isnegativecomment(1, 28) Then 
   if GetValue(1, 28) > 0 then  'only warn if tax amount > 0
      OnlyOneCheckOfTwo 1, 29, 30, ID_HOA_PERIOD_NONE_CHECKED, ID_HOA_PERIOD_MANY_CHECKED 
   end if 
  end if

  OnlyOneCheckOfThree 1, 31, 32, 33, ID_PROPERTY_RIGHTS_NONE_CHECKED, ID_PROPERTY_RIGHTS_MANY_CHECKED

  If isChecked(1, 33) Then
    Require IDOTHERDATA, 1, 34
  End If

  OnlyOneCheckOfThree 1, 35, 36, 37, ID_ASSIGNMENT_TYPE_NONE_CHECKED, ID_ASSIGNMENT_TYPE_MANY_CHECKED

  If isChecked(1, 37) Then
    Require IDOTHERDATA2, 1, 38
  End If

  Require IDLender, 1, 39
  Require IDLenderAdd, 1, 40
  OnlyOneCheckOfTwo 1, 41, 42, ID_SUBJECT_FOR_SALE_NONE_CHECKED, ID_SUBJECT_FOR_SALE_MANY_CHECKED
  Require ID_SUBJECT_BLOCK_DATA_SOURCE_REQUIRED, 1, 43
  If IsUAD and HasText(1, 43) Then
    ContractStr = UCase(GetText(1, 43))
    If InStr(ContractStr, "SUBJECT PROPERTY WAS OFFERED FOR SALE") > 0 Then
      If (InStr(ContractStr, ";MLS") = 0) and (InStr(ContractStr, "NOT LISTED ON MLS") = 0) Then
        AddRec ID_SUBJECT_BLOCK_DATA_SOURCE_NOTMLS, 1, 43
      End If
    End If
  End If
  If GetCellUADError(1, 43) Then
    AddRec ID_SUBJECT_BLOCK_DATA_SOURCE_UADERROR, 1, 43
  End If
  
  '****************************
  '****************************   FANNIE MAE RULES ADDED 12/04/2014

  
  'RULE #: FNM0083:
  if isChecked(1, 45) then 'DID NOT ANALYZED is checked
    if GetText(1, 46) = "" then
	  AddRec FNMCONTRACTNOTANALYZE, 1, 46
	end if
  end if
  
  'RULE # FNM0084: There was no comment on market conditions, even though one or more negative housing trends were indicated(declining, over supply, over 6 mos)
  if GetText(1, 87) = "" then
    count = GetCheck(1, 66) +  GetCheck(1, 69) + GetCheck(1, 72)
    if count > 0 then 'one or more negative trends
	  AddRec FNMMARKETCONDITION, 1, 85
    end if
  end if

  'RULE # FNM0087: Research of prior sale was not performed
  'pop up error even we have comment
  if isChecked(1, 45) then
    AddRec FNNOREARCHPRIORSALE, 1, 45
  end if
  
  'RULE # FNM0097: Illegal zoning compliance has been indicated in appraisal.
  if isChecked(1, 99) then
    AddRec FNMILLEGALZONING, 1, 99
  end if
  
 'RULE # FNM0099: Present use is indicated as not highest and best use.
 'if not isChecked(1, 101) then
 '  AddRec FNMBESTUSE, 1, 101
 'end if
  
  
 'RULE # FNM0176: The appraisal indicates that the subject property has legal nonconforming zoning
 ' Show ERROR when both legal Non Conforming and 'No' for Do the zoning regulations 
if isChecked(1, 95) and isChecked(1, 97) then
   AddRec FNMNONCONFORM, 1, 95
 end if
 

End Sub

Sub ReviewContract

  AddTitle String(5, "=") + "FNMA 1075 " + ID_CONTRACT_BLOCK + String(5, "=")

  If not isChecked(1, 36) and not isChecked(1, 37) Then
    ValidateStandardContractBlock 1, 44
    If hasText(1, 47) Then
      num = GetValue(1, 47)
      If num > GetValue(3, 268) Then
        AddRec IDCONTRACTFNMA1, 1, 47
      Else
        If num < GetValue(3, 268) Then
          AddRec IDCONTRACTFNMA2, 1, 47
        End If
      End If
    End If
  Else
    If hasText(1, 47) Then
      Require IDCONTRACTFNMA, 1, 35
    End If
  End If

  If GetCellUADError(1, 46) Then
    AddRec IDCONTRACTUADERROR, 1, 46
  End If

  OnlyOneCheckOfTwo 1, 52, 53, ID_CONTRACT_ASSISTANCE_NONE_CHECKED, ID_CONTRACT_ASSISTANCE_MANY_CHECKED
  CheckAndComment 1, 52, 54, ID_CONTRACT_ASSISTANCE_COMMENT_REQUIRED, ""
  If GetCellUADError(1, 54) Then
    AddRec IDCONTRACTUADERROR, 1, 54
  End If

End Sub


Sub ReviewNeighborhood

  AddTitle String(5, "=") + "FNMA 1075 " + IDNeigh + String(5, "=")
 
  ValidateStandardNeighborhoodBlock107310751004c 1, 55

  If hasText(3, 268) Then
    num = GetValue(3, 268) / 1000
      If num > GetValue(1, 74) Then
        AddRec IDTOBERANGEC, 1,74
      End If
  End If

  If hasText(3, 268) Then
    num = GetValue(3, 268) / 1000
      If num < GetValue(1, 73) Then
        AddRec IDTOBERANGEC, 1,73
      End If
  End If

  sum = GetValue(1, 79) + GetValue(1, 80) + GetValue(1, 81) + GetValue(1, 82) + GetValue(1, 84)
  if sum <> 100 then	
    AddRec IDLandUse, 1, 79
  end if

  Require IDNEIGHBOUND, 1, 85
  Require IDNEIGHDES, 1, 86
  Require IDNEIGHCOND, 1, 87

End Sub

Sub ReviewSite

  AddTitle String(5, "=") + "FNMA 1075 " + IDPRoSite + String(5, "=")

  Require IDTOPOGRAPHY, 1, 88
  Require IDSITEAREA, 1, 89
  If GetCellUADError(1, 89) Then
    AddRec IDSUBSITEUADERROR, 1, 89
  End If
  Require IDDENSITY, 1, 90
  Require IDVIEW, 1, 91
  If GetCellUADError(1, 91) Then
    AddRec IDSUBVIEWUADERROR, 1, 91
  End If
  Require IDZONINGCLAS, 1, 92
  Require ID_ZONING_DESCRIPTION_REQUIRED, 1, 93
  OnlyOneCheckOfFour 1, 94, 95, 98, 99, IDZONINGCOM, IDZONINGNOCHK   

  'RULE # FRE1093: Condominium Zoning - Triggers if the Site Zoning Compliance Type is Legal Nonconforming and the Site Zoning Permit Rebuild to Current Density Indicator is "No"
  Legal_Nonconforming = "Site Zoning is Legal Nonconforming.  Please ensure that this mortgage is eligible for sale to Freddie mac."
  if isChecked(1, 95) and isChecked(1, 97) then
    if isUAD then
      AddRec "*"&Legal_Nonconforming, 1, 95  'show critical warning
 	else
      AddRec Legal_Nonconforming, 1, 95
	end if
  end if
  
  
  
  If isChecked(1, 95) Then
    OnlyOneCheckOfTwo 1, 96, 97, IDNONCOFORM, "ZONING PERMITS REBUILDING: Both Yes and No are Checked"
  Else
    OnlyOneCheckOfTwo 1, 96, 97, "", "ZONING PERMITS REBUILDING: Both Yes and No are Checked"
  End if

  If isChecked(1, 99) Then
    Require IDILLEGAL, 1, 100
  End If

  ValidateSiteBlock 1, 101
            
End Sub

Sub ReviewImprovements

  AddTitle String(5, "=") + "FNMA 1075 " + IDIMPROVM2 + String(5, "=")

  Require IDDATASOURCE, 1, 133

  OnlyOneCheckOfSix 1, 134, 135, 136, 137, 138, 139, ID_PROJECT_DESCRIPTION_NONE_CHECKED, ID_PROJECT_DESCRIPTION_MANY_CHECKED
  CheckAndComment 1, 139, 140, "", ""
  Require IDSTORIES, 1, 141
  Require IDELEVATOR, 1, 142
  OnlyOneCheckOfThree 1, 143, 144, 145, IDEXISTING, "EXISTING/PROPOSED: More than one checked"
  Require IDYRBLT, 1, 146
  Require IDEFFAGE, 1, 147
  Require IDEXT, 1, 148
  Require IDROOFSUF, 1, 149
  Require IDTOTPARK, 1, 150
  Require IDRATIO, 1, 151
  Require IDTYPE, 1, 152
  Require IDGUEST, 1, 153

  '       Subject Phase
  Require IDSUBNUMPHASES, 1, 154
  Require IDSUBNUMUNITS, 1, 155
  Require IDSUBUNITSALE, 1, 156
  Require IDSUBUNITSOLD, 1, 157
  Require IDSUBUNITRENT, 1, 158
  Require IDSUBOWNEROCC, 1, 159

  '       If Proj Completed
  Require IDPCNUMPHASES, 1, 160
  Require IDPCNUMUNITS, 1, 161
  Require IDPCUNITSSALE, 1, 162
  Require IDPCUNITSSOLD, 1, 163 
  Require IDPCUNITSRENTED, 1, 164 
  Require IDPCOWNEROCC, 1, 165

  '       If Proj Incomplete
  Require IDPIPLANNED, 1, 166
  Require IDPIPLANNEDUNITS, 1, 167
  Require IDPIUNITESSALE, 1, 168
  Require IDPIUNITSSOLD, 1, 169
  Require IDPIUNITSRENT, 1, 170 
  Require IDPIOWNEROCC, 1, 171

  OnlyOneCheckOfThree 1, 172, 173, 174, IDOCCUPANCY, IDOCCUPANCYCK
  OnlyOneCheckOfTwo 1, 175, 176, IDHOANOCHK, IDHOACHK
  OnlyOneCheckOfThree 1, 177, 178, 179, IDMNGNOCHK, IDMNGCHK
  
  If isChecked(1, 179) Then
    Require IDMGTGRPNAME, 1, 180
  End If

  OnlyOneCheckOfTwo 1, 181, 182, ID_PROJECT_SINGLE_ENTITY_10_PERCENT_NONE_CHECKED, ID_PROJECT_SINGLE_ENTITY_10_PERCENT_MANY_CHECKED
  
  If isChecked(1, 181) Then
    Require ID_PROJECT_SINGLE_ENTITY_10_PERCENT_NONE_CHECKED_DESC, 1, 183
  End If

  OnlyOneCheckOfTwo 1, 184, 185, IDCONNOCHK, IDCONCHK
   
  If isChecked(1, 184) Then
    Require IDCONVERUSEANDDATE, 1, 186
  End If

  OnlyOneCheckOfTwo 1, 187, 188, IDCMMELENO, IDCMMELECHK

  If isChecked(1, 189) Then
    Require IDCOMELENO, 1, 189
  End If

  OnlyOneCheckOfTwo 1, 190, 191, ID_PROJECT_CONTAINS_COMMERCIAL_NONE_CHECKED, ID_PROJECT_CONTAINS_COMMERCIAL_MANY_CHECKED

  If isChecked(1, 190) Then
    Require IDCOMSPACENO, 1, 192
  End If
  If GetCellUADError(1, 192) Then
    AddRec IDCOMSPACEUADERROR, 1, 192
  End If

  Require IDFILENUM, 2, 2
  Require IDCONQUAL, 2, 5
  Require IDCOMELEREC, 2, 6 

  OnlyOneCheckOfTwo 2, 7, 8, IDLEASEDHOA, "COMM ELEM LEASED: More than one box checked"

  If isChecked(2, 7) Then
    Require IDLEASEDHOAYES, 2, 9
  End If

  OnlyOneCheckOfTwo 2, 10, 11, IDGROUNDRENT, "GROUND RENT: More than one box checked"
   
  If isChecked(2, 10) Then
    Require IDGROUNDRENTYES, 2, 12
  End If

  If isChecked(2, 10) Then
    Require IDGROUNDRENTYES2, 2, 13
  End If

  OnlyOneCheckOfTwo 2, 14, 15, IDPARKFAC, "PARKING FACILITIES: More than one box checked"
   
  If isChecked(2, 15) Then
    Require IDPARKFACNO, 2, 16
  End If

End Sub


Sub ReviewProjectAnalysis

   AddTitle String(5, "=") + "FNMA 1075 " + IDANALYSIS + String(5, "=")

   OnlyOneCheckOfTwo 2, 17, 18, IDANALYZEBUDGET, "PROJECT BUDGET: Both Did and Did Not analyze is checked"
   Require IDANALYZEBUDGETRESULTS, 2, 19
   OnlyOneCheckOfTwo 2, 20, 21, IDFACFEES, "ADDITIONAL FEES: Both Yes and No are checked"

   If isChecked(2, 20) Then
     Require IDFACFEESYES, 2, 22
   End If

   OnlyOneCheckOfThree 2, 23, 24, 25, IDUNITCHARGE, "UNIT CHARGES: More than one item checked"

   If isChecked(2, 23) Then
     Require IDUNITCHARGEHILO, 2, 26
   End If
   
   If isChecked(2, 25) Then
     Require IDUNITCHARGEHILO, 2, 26
   End If

   OnlyOneCheckOfTwo 2, 27, 28, IDUNUSUALCHAR, "SPECIAL CHARACTERITICS: Both Yes and NO are checked"

   If isChecked(2, 27) Then
     Require IDUNUSUALCHARYES, 2, 29
   End If

End Sub


Sub ReviewSubjectUnit
Dim Text1, QTYg, QTYcv, QTYop, ItemLoc

  AddTitle String(5, "=") + "FNMA 1075 " + IDROOMLIST2 + String(5, "=")

  Require IDUNITCHR, 2, 30
  Require IDPERYR, 2, 31
  Require IDCHARGEPERSQFTGLA, 2, 32
  OnlyOneCheckOfNine 2, 33, 34, 35, 36, 37, 38, 39, 40, 41, IDUTLNOCHK, XXXXX

  If isChecked(2, 41) Then
    Require IDMONTHLYOTHER, 2, 42
  End If

  If isChecked(2, 48) Then
    Require IDSOURCESOTHER, 2, 49
  End If

  Require IDDATASOURCE, 2, 50
  Require IDFLOOR, 2, 51
  Require IDLEVELS, 2, 52
  Require IDHEATTYPE, 2, 53
  Require IDHEATFUEL, 2, 54
  OnlyOneCheckOfThree 2, 55, 56, 57, IDCOOLCOND2, XXXXX

  If isChecked(2, 57) Then
    Require IDCOOLOTHER, 2, 58
  End If

  'Amenities
  OnlyOneCheckOfFive 2, 59, 61, 63, 65, 67, IDAMENITIES, XXXXX

  If isChecked(2, 59) Then
    If IsUAD Then
      If (not hasText(2, 60)) or (GetValue(2, 60) <= 0) Then
        AddRec IDFIREPLACE2, 2, 60
      End If
    Else
      Require IDFIREPLACE2, 2, 60
    End If
  Else
    If IsUAD and ((not hasText(2, 60)) or (GetValue(2, 60) <> 0)) then
      AddRec IDFIREPLACENONE, 2, 60
    End If
  End If

  If hasText(2, 60) Then
    If IsUAD Then
      If GetValue(2, 60) > 0 Then
        Require IDFIREPLACE2CK, 2, 59
      End If
    Else
      Require IDFIREPLACE2CK, 2, 59
    End If
  End If

  If isChecked(2, 61) Then
    If IsUAD Then
      If (GetValue(2, 62) <= 0) Then
        AddRec IDSTOVES2, 2, 62
      End If
    Else
      Require IDSTOVES2, 2, 62
    End If
  Else
    If IsUAD and ((not hasText(2, 62)) or (GetValue(2, 62) <> 0)) then
      AddRec IDSTOVESNONE, 2, 62
    End If
  End If

  If hasText(2, 62) Then
    If IsUAD Then
      If GetValue(2, 62) > 0 Then
        Require IDSTOVES2CK, 2, 61
      End If
    Else
      Require IDSTOVES2CK, 2, 61
    End If
  End If

  If isChecked(2, 63) Then
    If IsUAD Then
      If (not hasText(2, 64)) or (GetText(2, 64) = "None") Then
        AddRec IDPATDECK2, 2, 64
      End If
    Else
      Require IDPATDECK2, 2, 64
    End If
  Else
    If IsUAD and (GetText(2, 64) <> "None") then
      AddRec IDPATDECKNONE, 2, 64
    End If
  End If

  If hasText(2, 64) Then
    If IsUAD Then
      If GetText(2, 64) <> "None" Then
        Require IDPATDECKCK, 2, 63
      End If
    Else
      Require IDPATDECKCK, 2, 63
    End If
  End If

  If isChecked(2, 65) Then
    If IsUAD Then
      If (not hasText(2, 66)) or (GetText(2, 66) = "None") Then
        AddRec IDPORCH2, 2, 66
      End If
    Else
      Require IDPORCH2, 2, 66
    End If
  Else
    If IsUAD and (GetText(2, 66) <> "None") then
      AddRec IDPORCH2NONE, 2, 66
    End If
  End If

  If hasText(2, 66) Then
    If IsUAD Then
      If GetText(2, 66) <> "None" Then
        Require IDPORCH2CK, 2, 65
      End If
    Else
      Require IDPORCH2CK, 2, 65
    End If
  End If

  If isChecked(2, 67) Then
    If IsUAD Then
      If (not hasText(2, 68)) or (GetText(2, 68) = "None") Then
        AddRec IDOTHER2, 2, 68
      End If
    Else
      Require IDOTHER2, 2, 68
    End If
  Else
    If IsUAD and (GetText(2, 68) <> "None") then
      AddRec IDOTHERNONE, 2, 68
    End If
  End If

  If hasText(2, 68) Then
    If IsUAD Then
      If GetText(2, 68) <> "None" Then
        Require IDOTHER2CK, 2, 67
      End If
    Else
      Require IDOTHER2CK, 2, 67
    End If
  End If

  'Do not check for appliances on exterior only
  'OnlyOneCheckOfSix 2, 69, 70, 71, 72, 73, 74, IDAPPLIANCE, XXXXX

  checked = GetTextCheck(2, 76) + GetTextCheck(2, 77) + GetTextCheck(2, 78) + GetTextCheck(2, 80) + GetTextCheck(2, 81)
  if isChecked(2, 75) then
    if checked > 0 then AddRec IDCARNONE, 2, 75
  else
    if checked = 0 then AddRec IDCARERROR, 2, 75
  end if

  If isChecked(2, 76) Then
    Require IDGARNOCARS, 2, 79
  End If
  If IsUAD and hasText(3, 42) Then
    Text1 = GetText(3, 42)
    If (GetText(2, 79) <> "") Then
      If isChecked(2, 76) Then
        ItemLoc = InStr(1, Text1, "g")
        if (ItemLoc = 0) Then
          AddRec IDSUBGARAGEMISMATCH, 2, 76
        Else
          QtyTmp = Left(Text1, (ItemLoc - 1))
          If IsNumeric(QtyTmp) Then
            QTYg = CInt(QtyTmp)
          End If
          Text1 = Mid(Text1, (ItemLoc + 1))
        End If
      End If
      If isChecked(2, 77) Then
        ItemLoc = InStr(1, Text1, "cv")
        if (ItemLoc = 0) Then
          AddRec IDSUBCARSTORAGECOVEREDMISMATCH, 2, 77
        Else
          QtyTmp = Left(Text1, (ItemLoc - 1))
          If IsNumeric(QtyTmp) Then
            QTYcv = CInt(QtyTmp)
          End If
          Text1 = Mid(Text1, (ItemLoc + 2))
        End If
      End If
      If isChecked(2, 78) Then
        ItemLoc = InStr(1, Text1, "op")
        if (ItemLoc = 0) Then
          AddRec IDSUBCARSTORAGEOPENMISMATCH, 2, 78
        Else
          QtyTmp = Left(Text1, (ItemLoc - 1))
          If IsNumeric(QtyTmp) Then
            QTYop = CInt(QtyTmp)
          End If
          Text1 = Mid(Text1, (ItemLoc + 3))
        End If
      End If
      If GetValue(2, 79) <> QTYg + QTYcv + QTYop Then
        AddRec IDSUBCARSTORAGEQTY, 2, 79
      End If
    End If
  End If

  If isChecked(2, 76) Then
    OnlyOneCheckOfTwo 2, 80, 81, IDGARASSIGNOWN, XXXXX
  End If

  If isChecked(2, 80) Then
    Require IDGARSPACENO, 2, 82
  End If

  If isChecked(2, 81) Then
    Require IDGARSPACENO, 2, 82
  End If
 
  Require IDROOMCNT, 2, 83
  If GetCellUADError(2, 83) Then
    AddRec IDSUBROOMSUADERROR, 2, 83
  End If
  Require IDBEDCNT, 2, 84
  If GetCellUADError(2, 84) Then
    AddRec IDSUBBEDUADERROR, 2, 84
  End If
  Require IDBATHCNT, 2, 85
  If GetCellUADError(2, 85) Then
    AddRec IDSUBBATHSUADERROR, 2, 85
  End If
  
  Require IDGLA, 2, 86
  OnlyOneCheckOfTwo 2, 87, 88, IDHEATCOOLMETER, "INDIVIDUAL HEATING/COOLING: Both Yes and No are checked"

  If isChecked(2, 88) Then
    Require IDHEATCOOLMETERCOM, 2, 89
  End If

  Require IDADDFEACONDo, 2, 90
  Require IDCONDITIONCONDO, 2, 91
  If GetCellUADError(2, 91) Then
    AddRec IDCONDUADERROR, 2, 91
  End If
  OnlyOneCheckOfTwo 2, 92, 93, IDADVERSE2, "PHYSICAL DEFICIENCIES: Both Yes and No are checked"

  If isChecked(2, 92) Then
    Require IDADVERSE, 2, 94
  End If

  OnlyOneCheckOfTwo 2, 95, 96, IDCONFORM2, "PROPERTY CONFORMS: Both Yes and No are checked"

  If isChecked(2, 96) Then
    Require IDCONFORM, 2, 97
  End If

End Sub

Sub ReviewPriorSales
Dim GridIndex, FldCntr, CompAddrList, CompAddrSeq

   AddTitle String(5, "=") + "FNMA 1075 " + IDPRIORSALES + String(5, "=")

   OnlyOneCheckOfTwo 2, 98, 99, IDRESEARCH2, "SALES/TRANSFER HISTORY: Both Did and Did Not are checked"

   If isChecked(2, 99) Then
     Require IDRESEARCH, 2, 100
   End If

   OnlyOneCheckOfTwo 2, 101, 102, IDRESEARCHSUB2, "SUBJECT PRIOR HISTORY: Both Did and Did Not are checked"

   Require IDRESEARCHSUB3, 2, 103

   OnlyOneCheckOfTwo 2, 104, 105, IDRESEARCHCOMP2, "COMP PRIOR HISTORY: Both Did and Did Not are checked"

   Require IDRESEARCHCOMP3, 2, 106

   'Only check if I did check box is checked for the SUBJECT
   If isChecked(2, 101) Then	 
     If Not hasText(2, 107) Then
       AddRec IDDATEPSTSUB, 2, 107
     Else
       If IsUAD and (not IsDateOK(2, 107)) then
         AddRec IDDATEPSTFMTSUBJ, 2, 107
       End If
     End If
    If GetCellUADError(2, 107) Then
      AddRec IDDATEPSTSUBUADERROR, 2, 107
    End If

    Require IDPRICEPSTSUB, 2, 108
    Require IDDATASOURCESUB, 2, 109
    Require IDDATASOURCEDATESUB, 2, 110
    If GetCellUADError(2, 110) Then
      AddRec IDDATASRCDATESUBUADERROR, 2, 110
    End If

    If Not hasText(2, 111) Then
      AddRec IDDATEPSTCOMP1, 2, 111
    Else
      If IsUAD and (not IsDateOK(2, 111)) then
        AddRec IDDATEPSTFMTCOMP1, 2, 111
      End If
    End If
    If GetCellUADError(2, 111) Then
      AddRec IDDATEPSTCOMP1UADERROR, 2, 111
    End If

    Require IDPRICEPSTCOMP1, 2, 112
    Require IDDATASOURCECOMP1, 2, 113
    Require IDDATASOURCEDATECOMP1, 2, 114
    If GetCellUADError(2, 114) Then
      AddRec IDDATASRCDATECOMP1UADERROR, 2, 114
    End If

    If Not hasText(2, 115) Then
      AddRec IDDATEPSTCOMP2, 2, 115
    Else
      If IsUAD and (not IsDateOK(2, 115)) then
        AddRec IDDATEPSTFMTCOMP2, 2, 115
      End If
    End If
    If GetCellUADError(2, 115) Then
      AddRec IDDATEPSTCOMP2UADERROR, 2, 115
    End If

    Require IDPRICEPSTCOMP2, 2, 116
    Require IDDATASOURCECOMP2, 2, 117
    Require IDDATASOURCEDATECOMP2, 2, 118
    If GetCellUADError(2, 118) Then
      AddRec IDDATASRCDATECOMP2UADERROR, 2, 118
    End If

    If Not hasText(2, 119) Then
      AddRec IDDATEPSTCOMP3, 2, 119
    Else
      If IsUAD and (not IsDateOK(2, 119)) then
        AddRec IDDATEPSTFMTCOMP3, 2, 119
      End If
    End If
    If GetCellUADError(2, 119) Then
      AddRec IDDATEPSTCOMP3UADERROR, 2, 119
    End If

    Require IDPRICEPSTCOMP3, 2, 120
    Require IDDATASOURCECOMP3, 2, 121
    Require IDDATASOURCEDATECOMP3, 2, 122
    If GetCellUADError(2, 122) Then
      AddRec IDDATASRCDATECOMP3UADERROR, 2, 122
    End If
  End If 'end of check I did
  GridIndex = 0
  CompAddrList = Array(50,118,186)
  For FldCntr = 111 to 119 Step 4
    CompAddrSeq = CompAddrList(GridIndex)
    GridIndex = GridIndex + 1
    If (GetText(3, CompAddrSeq) <> "") or (GetText(3, (CompAddrSeq+1)) <> "") then
    'Only check if I did check box is checked for the COMPS
      If isChecked(2, 104) Then	
    	Require FormCompNumErrStr(IDDATEPSTCOMP1, 0, GridIndex), 2, (FldCntr)
        If GetCellUADError(2, (FldCntr)) Then
          AddRec FormCompNumErrStr(IDDATEPSTCOMP1UADERROR, 0, GridIndex), 2, (FldCntr)
        End If
        Require FormCompNumErrStr(IDPRICEPSTCOMP1, 0, GridIndex), 2, (FldCntr+1)
        Require FormCompNumErrStr(IDDATASOURCECOMP1, 0, GridIndex), 2, (FldCntr+2)
        Require FormCompNumErrStr(IDDATASOURCEDATECOMP1, 0, GridIndex), 2, (FldCntr+3)
        If GetCellUADError(2, (FldCntr+3)) Then
          AddRec FormCompNumErrStr(IDDATASRCDATECOMP1UADERROR, 0, GridIndex), 2, (FldCntr+3)
        End If
      End If 'end of ischecked
	End If
  Next

  Require IDANALYSIS2, 2, 123

End Sub

Sub ReviewSalesSubject
Dim GridIndex, FldCntr, num, num2, num3, num4, text1, text2, text3, text4, text5

  AddTitle String(5, "=") + "FNMA 1073 " + IDSALESAPP + String(5, "=")

  Require IDFILENUM, 3, 2
  Require IDPROPCOMP, 3, 5
  Require IDPROPCOMPFROM, 3, 6
  Require IDPROPCOMPTO, 3, 7
  Require IDSALECOMP, 3, 8
  Require IDSALECOMPFROM, 3, 9
  Require IDSALECOMPTO, 3, 10

    'Show warning if Settle date BEFORE contract date
  IsSettledDateBEFOREContractDate 3,63
  IsSettledDateBEFOREContractDate 3,131
  IsSettledDateBEFOREContractDate 3,199
  
  'Show error if Settle date is AFTER Effective date
  if not isUAD then
    IsSettledDateAFTEREffectiveDate 3, 21, 3, 269 'Subject  
  end if
  IsSettledDateAFTEREffectiveDate 3, 63, 3, 269  'Comp #1
  IsSettledDateAFTEREffectiveDate 3, 131, 3, 269 'Comp #2 
  IsSettledDateAFTEREffectiveDate 3, 199, 3, 269 'Comp #3 

  If hasText(3, 268) Then
    num = GetValue(3, 268)
    num2 = GetValue(3, 55)
    num3 = GetValue(3, 123)
    num4 = GetValue(3, 191)
    If num2 <= num3 and num2 <= num4 Then
      If num < num2 Then
        AddRec IDTOBERANGE2, 3, 55
       End If
     End If
    If num2 >= num3 and num2 >= num4 Then
      If num > num2 Then
        AddRec IDTOBERANGE2, 3, 55
      End If
    End If
    If num3 <= num2 and num3 <= num4 Then
      If num < num3 Then
        AddRec IDTOBERANGE2, 3, 123
      End If
    End If
    If num3 >= num2 and num3 >= num4 Then
      If num > num3 Then
        AddRec IDTOBERANGE2, 3, 123
      End If
    End If
    If num4 <= num3 and num4 <= num2 Then
      If num < num4 Then
        AddRec IDTOBERANGE2, 3, 191
      End If
    End If
    If num4 >= num3 and num4 >= num2 Then
      If num > num4 Then
        AddRec IDTOBERANGE2, 3, 191
      End If
    End If

    num2 = GetValue(3, 117)
    num3 = GetValue(3, 185)
    num4 = GetValue(3, 253)
    If num2 <= num3 and num2 <= num4 Then
      If num < num2 Then
        AddRec IDTOBERANGE3, 3, 117
      End If
    End If
    If num2 >= num3 and num2 >= num4 Then
      If num > num2 Then
        AddRec IDTOBERANGE3, 3, 117
      End If
    End If
    If num3 <= num2 and num3 <= num4 Then
      If num < num3 Then
        AddRec IDTOBERANGE3, 3, 185
      End If
    End If
    If num3 >= num2 and num3 >= num4 Then
      If num > num3 Then
        AddRec IDTOBERANGE3, 3, 185
      End If
    End If
    If num4 <= num3 and num4 <= num2 Then
      If num < num4 Then
       AddRec IDTOBERANGE3, 3, 253
      End If
    End If
    If num4 >= num3 and num4 >= num2 Then
      If num > num4 Then
        AddRec IDTOBERANGE3, 3, 253
      End If
    End If
  End If
  If hasText(3, 268) Then
    num = GetValue(3, 268)
    num2 = GetValue(3, 55)
    num3 = GetValue(3, 123)
    num4 = GetValue(3, 191)
    If num2 <= num3 and num2 <= num4 Then
      If num < num2 Then
        AddRec IDTOBERANGE2, 3, 55
       End If
     End If
    If num2 >= num3 and num2 >= num4 Then
      If num > num2 Then
        AddRec IDTOBERANGE2, 3, 55
      End If
    End If
    If num3 <= num2 and num3 <= num4 Then
      If num < num3 Then
        AddRec IDTOBERANGE2, 3, 123
      End If
    End If
    If num3 >= num2 and num3 >= num4 Then
      If num > num3 Then
        AddRec IDTOBERANGE2, 3, 123
      End If
    End If
    If num4 <= num3 and num4 <= num2 Then
      If num < num4 Then
        AddRec IDTOBERANGE2, 3, 191
      End If
    End If
    If num4 >= num3 and num4 >= num2 Then
      If num > num4 Then
        AddRec IDTOBERANGE2, 3, 191
      End If
    End If

    num2 = GetValue(3, 117)
    num3 = GetValue(3, 185)
    num4 = GetValue(3, 253)
    If num2 <= num3 and num2 <= num4 Then
      If num < num2 Then
        AddRec IDTOBERANGE3, 3, 117
      End If
    End If
    If num2 >= num3 and num2 >= num4 Then
      If num > num2 Then
        AddRec IDTOBERANGE3, 3, 117
      End If
    End If
    If num3 <= num2 and num3 <= num4 Then
      If num < num3 Then
        AddRec IDTOBERANGE3, 3, 185
      End If
    End If
    If num3 >= num2 and num3 >= num4 Then
      If num > num3 Then
        AddRec IDTOBERANGE3, 3, 185
      End If
    End If
    If num4 <= num3 and num4 <= num2 Then
      If num < num4 Then
       AddRec IDTOBERANGE3, 3, 253
      End If
    End If
    If num4 >= num3 and num4 >= num2 Then
      If num > num4 Then
        AddRec IDTOBERANGE3, 3, 253
      End If
    End If
  End If

  Require IDSUBADD, 3, 11
  Require IDSUBCITY, 3, 12

  If hasText(3, 12) Then
    text1 = Replace(UCase(GetText(3, 12)), " ", "")
    text2 = Replace(UCase(GetText(1, 9)), " ", "")
    text3 = Replace(UCase(GetText(1, 10)), " ", "")
    text4 = Replace(UCase(GetText(1, 11)), " ", "")
    text5 = Replace(UCase(GetText(1, 12)), " ", "")
    If IsUAD Then
      If not text1 = text2 + IDCOMMA + text3 + IDCOMMA + text4 + text5 Then
        AddRec IDADDMATCH, 3, 12
      End If
    End If
    If not IsUAD Then
      If not text1 = text3 + IDCOMMA + text4 + text5 Then
        AddRec IDADDMATCH, 3, 12
      End If
    End If
  End If
  If GetCellUADError(3, 11) or GetCellUADError(3, 12) Then
    AddRec IDADDRESSUADERROR, 3, 11
  End If

  Require IDSUBNAME, 3, 13
  Require IDSUBPHASE, 3, 14

  If not isChecked(1, 34) Then
    Require IDSUBSALES, 3, 15
    Require IDSUBPRGLA, 3, 16
  End If

  Require IDSUBLOC, 3, 22
  If GetCellUADError(3, 22) Then
    AddRec IDSUBLOCUADERROR, 3, 22
  End If
  Require IDSUBLEASEHOLD, 3, 23
  Require IDSUBHOA, 3, 24
  Require IDSUBCOMELE, 3, 25
  Require IDSUBRECFAC, 3, 26
  Require IDSUBFLOORLOC, 3, 27
  Require IDSUBVIEW, 3, 28
  If GetCellUADError(3, 28) Then
    AddRec IDSUBVIEWUADERROR, 3, 28
  End If
  Require IDSUBDESIGN, 3, 29
  Require IDSUBQUAL, 3, 30
  If GetCellUADError(3, 30) Then
    AddRec IDSUBQUALUADERROR, 3, 30
  End If
  Require IDSUBAGE, 3, 31
  If GetCellUADError(3, 31) Then
    AddRec IDSUBAGEUADERROR, 3, 31
  End If

  Require IDSUBCOND, 3, 32
  'Check Quality Condition between p1 cell 219 and p2 cell 27 make sure they are the same
  CheckQualityCondition 2, 91, 3, 32
  If GetCellUADError(3, 32) Then
    AddRec IDSUCONDUADERROR, 3, 32
  End If

  Require IDSUBROOMS, 3, 33
  If GetCellUADError(3, 33) Then
    AddRec IDSUBROOMSUADERROR, 3, 33
  End If
  Require IDSUBBED, 3, 34
  If GetCellUADError(3, 34) Then
    AddRec IDSUBBEDUADERROR, 3, 34
  End If
  Require IDSUBBATHS, 3, 35
  If GetCellUADError(3, 35) Then
    AddRec IDSUBBATHSUADERROR, 3, 35
  End If
  Require IDSUBGLA, 3, 36
  Require IDSUBBASE, 3, 37
  If GetCellUADError(3, 37) Then
    AddRec IDSUBBASEUADERROR, 3, 37
  End If
  If not ((GetText(3, 37) = "0sf") or (GetText(3, 37) = "0sqm")) Then
    Require IDSUBBASEPER, 3, 38
  End If
  If GetCellUADError(3, 38) Then
    AddRec IDSUBBASEPERUADERROR, 3, 38
  End If
  Require IDSUBFUNC, 3, 39
  Require IDSUBHEAT, 3, 40
  Require IDSUBENERGY, 3, 41
  If not IsEnergyEfficientOK(3, 41) then
    AddRec IDSUBENERGYBAD, 3, 41
  End If
  Require IDSUBGARAGE, 3, 42
  If IsUAD and hasText(3, 42) Then
    Text1 = GetText(3, 42)
    num = InStr(1, Text1, "g")
    num2 = InStr(1, Text1, "cv")
    num3 = InStr(1, Text1, "op")
    num4 = 0
    num5 = 0
    If not NumOrderIsOK(num, num2, num3, num4, num5) Then
      AddRec IDSUBCONDOFORMAT, 3, 42
    End If
  End If
  Require IDSUBPORCH, 3, 43

  GridIndex = 0
  For FldCntr = 50 to 186 Step 68
    GridIndex = GridIndex + 1
    If (GetText(3, (FldCntr)) <> "") or (GetText(3, (FldCntr+1)) <> "") then
      Require FormCompNumErrStr(IDCOMP1ADD, 0, GridIndex), 3, (FldCntr)
      Require FormCompNumErrStr(IDCOMP1UNITNO, 0, GridIndex), 3, (FldCntr+1)
      If GetCellUADError(3, (FldCntr)) or GetCellUADError(3, (FldCntr+1)) Then
        AddRec FormCompNumErrStr(IDCOMP1ADDRUADERROR, 0, GridIndex), 3, (FldCntr)
      End If
      Require FormCompNumErrStr(IDCOMP1NAME, 0, GridIndex), 3, (FldCntr+2)
      Require FormCompNumErrStr(IDCOMP1PHASE, 0, GridIndex), 3, (FldCntr+3)
      Require FormCompNumErrStr(IDCOMP1PROX, 0, GridIndex), 3, (FldCntr+4)
      Require FormCompNumErrStr(IDCOMP1SALES, 0, GridIndex), 3, (FldCntr+5)

      ' Check comp one-unit housing & comp sales ranges
      If hasText(3, (FldCntr+5)) Then
        num2 = GetValue(3, (FldCntr+5))
        num = num2 / 1000
        If num < GetValue(1, 71) Then
          AddRec FormCompNumErrStr(IDTOBERANGE4, 0, GridIndex), 3, (FldCntr+5)
        Else
          If num > GetValue(1, 72) Then
            AddRec FormCompNumErrStr(IDTOBERANGE4, 0, GridIndex), 3, (FldCntr+5)
          End If
        End If
        If num2 < GetValue(3,9) Then
          AddRec FormCompNumErrStr(IDTOBERANGE10, 0, GridIndex), 3, (FldCntr+5)
        Else
          If num2 > GetValue(3,10) Then
            AddRec FormCompNumErrStr(IDTOBERANGE10, 0, GridIndex), 3, (FldCntr+5)
          End If
        End If
      End If

      Require FormCompNumErrStr(IDCOMP1PRGLA, 0, GridIndex), 3, (FldCntr+6)
      Require FormCompNumErrStr(IDCOMP1SOURCE, 0, GridIndex), 3, (FldCntr+7)
      If GetCellUADError(3, (FldCntr+7)) Then
        AddRec FormCompNumErrStr(IDCOMP1DATASRCUADERROR, 0, GridIndex), 3, (FldCntr+7)
      End If
      Require FormCompNumErrStr(IDCOMP1VER, 0, GridIndex), 3, (FldCntr+8)
      Require FormCompNumErrStr(IDCOMP1SF, 0, GridIndex), 3, (FldCntr+9)
      If GetCellUADError(3, (FldCntr+9)) Then
        AddRec FormCompNumErrStr(IDCOMP1SFUADERROR, 0, GridIndex), 3, (FldCntr+9)
      End If
      Require FormCompNumErrStr(IDCOMP1CONC, 0, GridIndex), 3, (FldCntr+11)
      If GetCellUADError(3, (FldCntr+11)) Then
        AddRec FormCompNumErrStr(IDCOMP1CONCUADERROR, 0, GridIndex), 3, (FldCntr+11)
      End If
      Require FormCompNumErrStr(IDCOMP1DOS, 0, GridIndex), 3, (FldCntr+13)
      If GetCellUADError(3, (FldCntr+13)) Then
        AddRec FormCompNumErrStr(IDCOMP1DOSUADERROR, 0, GridIndex), 3, (FldCntr+13)
      End If
      Require FormCompNumErrStr(IDCOMP1LOC, 0, GridIndex), 3, (FldCntr+15)
      If GetCellUADError(3, (FldCntr+15)) Then
        AddRec FormCompNumErrStr(IDCOMP1LOCUADERROR, 0, GridIndex), 3, (FldCntr+15)
      End If
      Require FormCompNumErrStr(IDCOMP1LEASEHOLD, 0, GridIndex), 3, (FldCntr+17)
      Require FormCompNumErrStr(IDCOMP1HOA, 0, GridIndex), 3, (FldCntr+19)
      Require FormCompNumErrStr(IDCOMP1COMELE, 0, GridIndex), 3, (FldCntr+21)
      Require FormCompNumErrStr(IDCOMP1RECFAC, 0, GridIndex), 3, (FldCntr+23)
      Require FormCompNumErrStr(IDCOMP1FLOORLOC, 0, GridIndex), 3, (FldCntr+25)
      Require FormCompNumErrStr(IDCOMP1VIEW, 0, GridIndex), 3, (FldCntr+27)
      If GetCellUADError(3, (FldCntr+27)) Then
        AddRec FormCompNumErrStr(IDCOMP1VIEWUADERROR, 0, GridIndex), 3, (FldCntr+27)
      End If
      Require FormCompNumErrStr(IDCOMP1DESIGN, 0, GridIndex), 3, (FldCntr+29)
      Require FormCompNumErrStr(IDCOMP1QUAL, 0, GridIndex), 3, (FldCntr+31)
      If GetCellUADError(3, (FldCntr+31)) Then
        AddRec FormCompNumErrStr(IDCOMP1QUALUADERROR, 0, GridIndex), 3, (FldCntr+31)
      End If
      Require FormCompNumErrStr(IDCOMP1AGE, 0, GridIndex), 3, (FldCntr+33)
      If GetCellUADError(3, (FldCntr+33)) Then
        AddRec FormCompNumErrStr(IDCOMP1AGEUADERROR, 0, GridIndex), 3, (FldCntr+33)
      End If
      Require FormCompNumErrStr(IDCOMP1COND, 0, GridIndex), 3, (FldCntr+35)
      If GetCellUADError(3, (FldCntr+35)) Then
        AddRec FormCompNumErrStr(IDCOMP1CONDUADERROR, 0, GridIndex), 3, (FldCntr+35)
      End If
      Require FormCompNumErrStr(IDCOMP1ROOMS, 0, GridIndex), 3, (FldCntr+38)
      If GetCellUADError(3, (FldCntr+38)) Then
        AddRec FormCompNumErrStr(IDCOMP1ROOMSUADERROR, 0, GridIndex), 3, (FldCntr+38)
      End If
      Require FormCompNumErrStr(IDCOMP1BED, 0, GridIndex), 3, (FldCntr+39)
      If GetCellUADError(3, (FldCntr+39)) Then
        AddRec FormCompNumErrStr(IDCOMP1BEDUADERROR, 0, GridIndex), 3, (FldCntr+39)
      End If
      Require FormCompNumErrStr(IDCOMP1BATHS, 0, GridIndex), 3, (FldCntr+40)
      If GetCellUADError(3, (FldCntr+40)) Then
        AddRec FormCompNumErrStr(IDCOMP1BATHSUADERROR, 0, GridIndex), 3, (FldCntr+40)
      End If

      Require FormCompNumErrStr(IDCOMP1GLA, 0, GridIndex), 3, (FldCntr+42)
      ' Check if the comp's GLA is 20% more than the subject GLA
      If hasText(3, 36) and hasText(3, (FldCntr+42)) then  'we have subject & comp GLA
        cGLA = GetValue(3, (FldCntr+42))
        sGLA = GetValue(3, 36)

        If sGLA > 0 then
          num = (cGLA - sGLA)/sGLA
          If (Abs(num) > 0.20) and (cGLA > sGLA)then
            AddRec FormCompNumErrStr(IDCOMPGLA20RANGE1G, 0, GridIndex), 3, (FldCntr+42)
          End If
          If (ABs(num) > 0.20) and (cGLA < sGLA) then
            AddRec FormCompNumErrStr(IDCOMPGLA20RANGE1L, 0, GridIndex), 3, (FldCntr+42)
          End If
        End If
      End If

      Require FormCompNumErrStr(IDCOMP1BASE, 0, GridIndex), 3, (FldCntr+44)
      If GetCellUADError(3, (FldCntr+44)) Then
        AddRec FormCompNumErrStr(IDCOMP1BASEUADERROR, 0, GridIndex), 3, (FldCntr+44)
      End If

      If not ((GetText(3, (FldCntr+44)) = "0sf") or (GetText(3, (FldCntr+44)) = "0sqm")) Then
        Require FormCompNumErrStr(IDCOMP1BASEPER, 0, GridIndex), 3, (FldCntr+46)
      End If
      If GetCellUADError(3, (FldCntr+46)) Then
        AddRec FormCompNumErrStr(IDCOMP1BASEPERUADERROR, 0, GridIndex), 3, (FldCntr+46)
      End If
      Require FormCompNumErrStr(IDCOMP1FUNC, 0, GridIndex), 3, (FldCntr+48)
      Require FormCompNumErrStr(IDCOMP1HEAT, 0, GridIndex), 3, (FldCntr+50)
      Require FormCompNumErrStr(IDCOMP1ENERGY, 0, GridIndex), 3, (FldCntr+52)
      If not IsEnergyEfficientOK(3, (FldCntr+52)) then
        AddRec FormCompNumErrStr(IDCOMP1ENERGYBAD, 0, GridIndex), 3, (FldCntr+52)
      End If
      Require FormCompNumErrStr(IDCOMP1GARAGE, 0, GridIndex), 3, (FldCntr+54)
      If IsUAD and hasText(3, (FldCntr+54)) Then
        Text1 = GetText(3, (FldCntr+54))
        num = InStr(1, Text1, "g")
        num2 = InStr(1, Text1, "cv")
        num3 = InStr(1, Text1, "op")
        num4 = 0
        num5 = 0
        If not NumOrderIsOK(num, num2, num3, num4, num5) Then
          AddRec FormCompNumErrStr(IDCOMP1CONDOFORMAT, 0, GridIndex), 3, (FldCntr+54)
        End If
      End If
      Require FormCompNumErrStr(IDCOMP1PORCH, 0, GridIndex), 3, (FldCntr+56)

      If hasText(3, (FldCntr+67)) Then
        num = GetValue(3, (FldCntr+67)) / 1000
          If num < GetValue(1, 73) Then
            AddRec FormCompNumErrStr(IDTOBERANGE5, 0, GridIndex), 3, (FldCntr+67)
          Else
            If num > GetValue(1, 74) Then
              AddRec FormCompNumErrStr(IDTOBERANGE5, 0, GridIndex), 3, (FldCntr+67)
            End If
          End If
      End If
    End If
  Next

  Require IDSUMMARYSALES, 3, 254

  If Not hasText(3, 255) Then
    AddRec IDTOBE, 3, 255
  End If

End Sub


Sub ReviewIncomeApproach

  AddTitle String(5, "=") + "FNMA 1073 " + IDINCOME + String(5, "=")

  Require IDINCOMERENT2, 3, 256
  Require IDINCOMEGRM2, 3, 257
  Require IDINCOMEVALUE2, 3, 258
  Require IDSUMMARYINCOME, 3, 259

End Sub


Sub ReviewReconciliation

  AddTitle String(5, "=") + "FNMA 1073 " + IDRECON + String(5, "=")

  If Not hasText(3, 260) Then
    AddRec IDTOBE, 3, 260
  End If

  Require IDINCOMEVALUE2, 3, 261
  OnlyOneCheckOfFour 3, 263, 264, 265, 266, IDSTATUSNOCHK, IDSTATUSCHK

  If isChecked(3, 265) Then
    Require IDCONDCOMM, 3, 267
  End If

  If isChecked(3, 266) Then
    Require IDCONDCOMM, 3, 267
  End If

  If Not hasText(3, 268) Then
    AddRec IDTOBE, 3, 268
  End If

  Require IDASOF, 3, 269
  
 '###################  PCV Accuracy 3 Revised  #####################
'PCVAC-33: Fireplace(s) box is checked but # of Fireplace(s) is not more than "0".  Provide number of Fireplaces.
PCVAC33Msg = "** Fireplace(s) box is checked but # of Fireplace(s) is not more than '0'.  Provide number of Fireplaces."
if isChecked(2, 59) then
  if GetValue(2, 60) = 0 then
    AddRec PCVAC33Msg, 2, 60
  end if 	
end if

'PCVAC-34: Number of Fireplace(s) is more than "0", but Fireplace(s) box is not checked.  Correction required
PCVAC34Msg = "** Number of Fireplace(s) is more than '0', but Fireplace(s) box is not checked.  Correction required"
if GetValue(2, 60) > 0 then
  if not isChecked(2, 59) then
    AddRec PCVAC34Msg, 2, 59
  end if
end if

'PCVAC-35: Number of Woodstove(s) is more than "0", but Woodstove(s) box is not checked.  Correction required
PCVAC35Msg = "** Number of Woodstove(s) is more than '0', but Woodstove(s) box is not checked.  Correction required"
if GetValue(2, 62) > 0 then
  if not isChecked(2, 61) then
    AddRec PCVAC35Msg, 2, 61
  end if 
end if

'PCVAC-36: Other box is checked but description field is blank.  Comment required, describe other amenity.
PCVAC36Msg = "** Other box is checked but description field is blank.  Comment required, describe other amenity."
if isChecked(2, 67) then
  if GetText(2, 68) = "" then
    AddRec PCVAC36Msg, 2, 68
  end if
end if

'PCVAC-37: Woodstove(s) box is checked but # of woodstoves is not more than "0".  Provide number of woodstoves.
PCVAC37Msg = "** Woodstove(s) box is checked but # of woodstoves is not more than '0'.  Provide number of woodstoves."
if isChecked(2, 61) then
  if GetValue(2, 62) = 0 then
    AddRec PCVAC37Msg, 2, 62
  end if 
end if

'PCVAC-47: Number of Bath(s) field is "N/A".  Provide number of baths.
PCVAC47Msg = "** Number of Bath(s) field is 'N/A'.  Provide number of baths."
bath = GetText(2, 85)
bath = trim(UCase(bath))
if bath = "N/A" then
  AddRec PCV47Msg, 2, 85
end if

'PCVAC-48: Number of Bath(s) field is blank.  Provide number of baths.
PCVAC48Msg = "** Number of Bath(s) field is blank.  Provide number of baths."
if GetText(2, 85) = "" then
  AddRec PCVAC48Msg, 2, 85
end if

'PCVAC-49: Number of Bedrooms field is "N/A".  Provide number of bedrooms.
PCVAC49MSG = "** Number of Bedrooms field is 'N/A'.  Provide number of bedrooms."
bed = GetText(2, 84)
bed = trim(UCase(bed))
if bed = "N/A" then
  AddRec PCV49Msg, 2, 84
end if

'PCVAC-50: Number of Bedrooms field is blank.  Provide number of bedrooms.
PCVAC50Msg = "** Number of Bedrooms field is blank.  Provide number of bedrooms."
if GetText(2, 84) = "" then
  AddRec PCVAC50Msg, 2, 84 
end if





 

End Sub
