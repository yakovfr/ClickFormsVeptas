unit UCC_Data_MobileInspection;
...
begin
  result := False;
  UserName := CurrentUser.AWUserInfo.FLoginName;
  Password := CurrentUser.AWUserInfo.UserPassword;
  mi := MobileInspection.Create(UserName, Password);
  try
...
begin
  ...
  mi := MobileInspection.Create(UserName, CurrentUser.AWUserInfo.UserPassword);
  try
...
begin
  result := False;  aInt := 0;
  UploadInfo2.UserName := CurrentUser.AWUserInfo.FLoginName;
...
//  jsPostRequest.Add('DataStructureVersionNumber',JSON_VERSION_NO);
  ...
  //the jsInspAll includes the blob data of Subject, Comps, Listings,  and Summary
  jsInspALL := TlkJSONObject.Create(true);
  jsInspAll.Add('DataStructureVersionNumber',JSON_VERSION_NO);
  if jsSubject <> nil then
    jsInspAll.Add('Subject', jsSubject);
  if jsComps <> nil then
    jsInspAll.Add('Comps',jsComps);
