...
uses
  ...
  Printers, uWindowsInfo,UCC_Data_MobileInspection,
  IdHTTP, IdContext, IdMultipartFormData, uGraphics,
  IdSSLOpenSSL, xmldom, XMLIntf, UForms, AWSI_Server_Access,UGlobals,
  msxmldom, XMLDoc,UCC_Globals,uUADObject,IniFiles,UCellMetaData, uMessages,
  RzLabel;
...
//    FCompBasePanelHeight: Integer;  //original base panel height for comps
    ...
    procedure DownloadSketchMdataJson(joSketchData: TlkJSonBase; thisForm: TDocForm);
...
//    procedure LoadInspectionData_SubjectData(subject_data:String; var FSubjectData:TSubjectInspectData);
    ...
    procedure SaveToCell(SketchXMLFile: String; FCell:TSketchCell; PageNo:Integer);  //copy over the XML structure to the Sketch Cell
...
uses
  ...
  UCC_Mobile_Inspection, USketch_JSonToXML, UUADUtils, UOrderManager;
...
//  LoadInspectionData_SubjectData(subject_data, FSubjectData);
  LoadInspectionData_SubjectData(subject_data);
//    FormID := GetFormIDbyJobType(FSubjectData.insp_type); //use job type to look up the form id to use
  ...
  if FSketchJSon <> '' then
   ...
   /// for f := 0 to FDoc.docForm.Count - 1 do  //Ticket #1359: need to comment this routine out. Per Jeff, no need to call populate context id since the post process already did.
   ///   if assigned(FDoc.docForm[f]) then
...
begin
  ...
  With PrefFile do
    begin
      ...
      WriteInteger('Inspection', 'DataStructureVersionID', FVersionNumber);
...
begin
  ...
  for i:= 0 to TopPhotoSection.ComponentCount -1 do
    begin
      aName := GetNameByTitle(aTitle);   //Ticket #1407
...
begin
  ...
  if (idx >= 0) and not FOverwriteData then
    begin
            ...
            if aEdit is TEdit then
              if (aTag <= FSubjectPhotoList.count) and Assigned(FSubjectPhotoList.Items[aTag]) then  //make sure we have a photo
                begin
                  ...
                  ImportSubjectExtraImages(TImage(aImagePhoto),0, TEdit(aEdit).Text);
                 // ImportSubjectExtraImages(TImage(aImagePhoto),i, TEdit(aEdit).Text);
                end;
          end;
      end;
  except on E:Exception do
...
begin
    ...
    if idx = -1 then
      ...
      aForm := FDoc.GetFormByOccuranceByIdx(aFormID, 0, idx, False);
    if assigned(aForm) and not FOverwriteData then
      begin
      ...
      begin
        if isNew then
          ...
          begin
            ...
            if not found then
              begin
                cell := aForm.GetCell(1, 21);
                if assigned(cell) and (cell is TPhotoCell) and (not TPhotoCell(cell).HasImage or FOverWriteData) then
                  begin
                    aCellID := 21;
                    aCellTitleID := aCellID - 2;
                    found := True;
      ...
      if jpostedData.Count = 0 then
        ...
        begin
          ...
          if pos('DataStructureVersionNumber',inspection_data) > 0 then
            FVersionNumber := FjPostedData.Field['DataStructureVersionNumber'].Value
          else
            FVersionNumber := 1;
...
//          LoadInspectionData_SubjectData(subject_data, FSubjectData);
        ...
        comps_data := TlkJSON.GenerateText(jComps);
//        ImportCompDataToReport(comps_data);
//showmessage(Format('FSalesCount=%d FListCount = %d',[FSaleCount, FListCount]));
        //Load subject images
...
try
  ...
  if FSubjectData2.DrivewayCars <> -1 then
    SetCellTextByCellID(360, Format('%d',[FSubjectData2.DrivewayCars]))
  else //github #663
    begin
  ...
  if FSubjectData2.GarageCars <> -1 then  //when value = -1 means NULL or EMPTY from the mobile
    SetCellTextByCellID(2030, Format('%d',[FSubjectData2.GarageCars]))
  else //github #663
    begin
      if Fdoc.UADEnabled and (ga_CellValue = 0) then
        begin
          SetCellTextByCellID(2030, '0', True) //if it's NULL or EMPTY set to 0 for garage car count to avoid UAD review error
        end
      else if ga_CellValue > 0 then
        begin
          SetCellTextByCellID(2030, Format('%d',[ga_CellValue]));
          SetCellTextByCellID(349, 'X');
        end;
    end;
   if FSubjectData2.GarageCars > 0 then
     begin
     //  if FSubjectData2.GarageIsAttached  or FSubjectData2.GarageAttachedCars then
        if FSubjectData2.GarageAttachedCars then //Ticket #1325: use GarageAttachedCars instead
         SetCellTextByCellID(2070, 'X')

     //  else if FSubjectData2.GarageIsDetached or FSubjectData2.GarageDetachedCars then
       else if FSubjectData2.GarageDetachedCars then
         SetCellTextByCellID(2071, 'X')


     // else if FSubjectData2.GarageIsBuiltin or FSubjectData2.GarageBuiltincars then
  ...
  //Carport
  if FSubjectData2.CarportCars <> -1 then
    SetCellTextByCellID(355, Format('%d',[FSubjectData2.CarportCars]))
  else //github #663
    begin
      ...
      else if cp_CellValue > 0 then
        begin
          SetCellTextByCellID(355, Format('%d',[cp_CellValue]));
          SetCellTextByCellID(2657, 'X');
        end;
    end;
//  c1 := GetValidInteger(FDoc.GetCellTextByID(355));
//  c1label.Caption := Format('%d',[c1]);
  if FSubjectData2.CarportCars > 0 then
    begin
      SetCellTextByCellID(2657, 'X');
      if FSubjectData2.GarageCars <= 0 then //Ticket #1155: only check attached/detach if no car garage
        begin
    ...
    aGarageCellValue := '';
    aGarage := FSubjectData2.GarageCars;
    if aGarage = - 1 then
      aGarage := GetValidInteger(FDoc.GetCellTextByID(2030));
    if aGarage > 0 then
      begin
//        if FSubjectData2.GarageAttachedCars or FSubjectData2.GarageIsAttached then
        if FSubjectData2.GarageAttachedCars then  //Ticket #1325: use GarageAttachedCars instead
          SetGarageCellValue(aGarage, 'ga', aGarageCellValue)

//        else if FSubjectData2.GarageDetachedCars or FSubjectData2.GarageIsDetached then
        else if FSubjectData2.GarageDetachedCars then
          SetGarageCellValue(FSubjectData2.GarageCars, 'gd', aGarageCellValue)

//        else if FSubjectData2.GarageBuiltincars or FSubjectData2.GarageIsBuiltin then
        else if FSubjectData2.GarageBuiltincars then
          SetGarageCellValue(aGarage, 'gbi', aGarageCellValue)
        else
          SetGarageCellValue(aGarage, 'ga', aGarageCellValue);   //default to ga if all 3 options are empty
    ...
    aCarport := FSubjectData2.CarportCars;
    if aCarport = -1 then
      aCarport := cp_CellValue;   //use cell value
    if aCarport > 0 then //if we have carpot add to the gararacellvalue
      begin
       // if (FSubjectData2.CarportIsAttached or FSubjectData2.CarportAttachedCars) and (FSubjectData2.GarageCars = 0) then
        if FSubjectData2.CarportAttachedCars and (FSubjectData2.GarageCars = 0) then //Ticket #1325
          SetGarageCellValue(aCarport, 'cpa', aGarageCellValue)
       // else if (FSubjectData2.CarportIsDetached or FSubjectData2.CarportDetachedCars)  and (FSubjectData2.GarageCars = 0) then
        else if FSubjectData2.CarportDetachedCars  and (FSubjectData2.GarageCars = 0) then
          SetGarageCellValue(aCarport, 'cpd', aGarageCellValue)
        else
          SetGarageCellValue(aCarport, 'cp', aGarageCellValue);
      end;
    aDriveWay := FSubjectData2.DrivewayCars;
    if aDriveWay = -1 then //if empty or null driveway from the mobile, use the cell value
      aDriveWay := dw_CellValue;
    ...
    if assigned(FDocCompTable) then
      ImportGridData(0, FDoc, '1016', aGarageCellValue, FDocCompTable);
    if GetValidInteger(FDoc.GetCellTextByID(355)) = 0 then
      SetCellTextByCellID(355, Format('%d',[aCarport]), True);
    if GetValidInteger(FDoc.GetCellTextByID(360)) = 0 then
      SetCellTextByCellID(360, Format('%d',[aDriveway]), True);
    if GetValidInteger(FDoc.GetCellTextByID(2030)) = 0 then
      SetCellTextByCellID(2030, Format('%d',[aGarage]), True);
  //Condition Rating
  if FSubjectData2.ConditionRating > 0 then
    SetCellTextByCellID(998, Format('C%d',[FSubjectData2.ConditionRating]))
  else
    SetCellTextByCellID(998, '');
//PAM: Ticket#:???? - donot use ConditionCommentDescription to set cell id 520
//We only use this field to push down to mobile app,
//from mobile app, we use 4 different fields to populate the kitchen & bath imporovement
//  SetCellTextByCellID(520, FSubjectData2.ConditionCommentDescription);
  SetCellTextByCellID(471, FSubjectData2.AdverseConditionsDescription);
//github #891
//  SetCellTextByCellID(601, FSubjectData2.ConditionCommentNeighborhood);
  ...
  //if FSubjectData2.GLA <> -1 then
  //  SetCellTextByCellID(232, Format('%d',[FSubjectData2.GLA]));
 // SetCellTextByCellID(232, FSubjectData2.GLA);
 ...
 //  SetCellTextByCellID(7, FSubjectData2.PrimaryName);      //github 674
 //  SetCellTextByCellID(14, FSubjectData2.PrimaryHomeNo);

   //Exterior...
//  c4 := GetValidInteger(FDoc.GetCellTextByID(355));
//  c4label.Caption := Format('%d',[c4]);
...
const
 ...
 var
  ...
  sketcher_page_count,sketcher_packet_version: String;
  sketcher_mdata_version,sketchMdata: String;
  js_image_array, js_Sketch_data, js_Sketch_array, js_Sketch_MData_Array: TlkJSonBase;
...
try
  try
    if FOverWriteData then
      begin
      //  DeleteExistingForm(FDoc, SketchFormID);   //github #744: donot remove any existing sketch
      //  DeleteExistingForm(FDoc, SketchFormID2);
      end;
...
procedure TInspectionDetail.DownloadSketchMdataJson(joSketchData: TlkJSonBase; thisForm: TDocForm);
const
  SketchformID = 4177;
                ...
                begin
                  if strValue <> '' then
                    begin
                      ...
                      FSubjectData.TotalArea := strValue;
                      SetCellTextByCellID(200, FSubjectData.TotalArea);
                      SetCellTextByCellID(201, FSubjectData.FinishedPercent);
                      if (GetValidInteger(FSubjectData.TotalArea) > 0) then
                        begin
                          aFinishArea := CalcFinishedArea(FSubjectData.TotalArea, FSubjectData.FinishedPercent);
                          if FSubjectData.AccessMethodWalkUp then
...
procedure TInspectionDetail.SaveToCell(SketchXMLFile: String; FCell:TSketchCell; pageNo:Integer);  //copy over the XML structure to the Sketch Cell
var
  SketchXML: TXMLDocument;
  xmlStr, skName: String;
begin
  skName := formatdatetime('mdyyhns',now);
  if not FileExists(SketchXMLFile) then
    exit;

  try
    SketchXML := TXMLDocument.Create(Application.MainForm);
    SketchXML.DOMVendor := GetDomVendor('MSXML');
    SketchXML.FileName := SketchXMLFile;
...
begin
  ...
  aTitle := '';
  case i of
    1: aFormID := SketchFormID;
    else
       aFormID := SketchFormID2;
  end;
  //look for sketch form in the container
(*
  for cntr := 0 to FDoc.docForm.Count - 1 do
    if FDoc.docForm[cntr].FormID = aFormID then
      begin
        thisForm := FDoc.docForm[cntr];
//        thisForm := FDoc.GetFormByOccurance(FDoc.docForm[cntr].FormID, js.count-1, True);
        cell := thisForm.GetCellByID(cidSketchImage);   //sketch cell id: 1157
        if assigned(cell) and (cell is TSketchCell) then
          begin
            if TSketchCell(cell).HasImage then
              begin
                if FOverwriteData then
                  //if Ok2Continue('You''re about to overwrite the existing sketch.  Continue?') then
                    TSketchCell(cell).Clear
                else
                  Exit;
              end;
//          break;
...
//  if i < length(skCells) then  //we already have available sketch form in the report
  ...
  if assigned(cell) and (cell is TSketchCell) then
    begin
       ...
       try
         ...
         cell.Display;
//         if aFormID = SketchFormID then
           begin
             case FVersionNumber of
               1: DownloadSketchMdataJson(jsMeta, thisForm);
               else DownloadSketchMdataJson2(jsMeta, thisForm, TSketchCell(cell),i);
             end;
           end;
...
(*
  if (FOverwriteData or (assigned(TSketchCell(cell)) and  TSketchCell(cell).FEmptyCell)) then  //1st sketch, put metadata there
    with TSketchCell(cell) do
      begin
        FMetaData := TSketchData.Create(mdAreaSketchData,1,sketchLabel); //create meta storage
        FMetaData.FUID := mdAreaSketchData;
        FMetaData.FVersion := 1;
        FMetaData.FData := TMemoryStream.Create;      //create new data storage
        FMetaData.FData.CopyFrom(aStream, 0);  //save to cells metaData
      end;
*)
       finally
         aStream.Free;
    ...
    begin
//      showmessage('Cannot insert subject photo');
    end;
end;
...
begin
//  CompBasePanel.Width := self.width;
  ...
  for i := 0 to doc.docForm.Count - 1 do
    begin
      case doc.docForm.Forms[i].FormID of
        340, 355, 342, 889, 11, 4220, 4218: //both forms are using the same xcomp page 363
          begin
            formID := 363;
            found := true;
            break;
...
begin
  ...
  for i := 0 to doc.docForm.Count - 1 do
    begin
      case doc.docForm.Forms[i].FormID of
        340, 355, 4220, 4218: //both forms are using the same xcomp page 363
          begin
            formID := 3545;
            found := true;
            break;
