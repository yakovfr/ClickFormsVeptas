...
uses
  ...
  UUtil1, UCC_Data_MobileInspection,UMobile_Utils,
  ...
  RzButton, RzPanel, UCC_InspectionDetail;
...
type
  ...
  private
    ...
    procedure CheckAvailableLatestVersions;
...
try
//  FOverride    := True;
  FUserID      := CurrentUser.AWUserInfo.FLoginName;
  FUserPsw     := CurrentUser.AWUserInfo.FPassword;
...
//github #656: do not bring in empty 1004 form
///      FDoc.InsertBlankUID(TFormUID.Create(340), true, -1, False); //create it
    end;
//github #656: do not bring in empty 1004 form
///  if FDoc.FormCount = 0 then
///    FDoc.InsertBlankUID(TFormUID.Create(340), true, -1, False); //create it

  FDocCompTable := TCompMgr2.Create(True);
//github #656: do not bring in empty 1004 form and do not buildgrid here
...
begin
  ...
  try
    ...
    begin
        ...
        if FTeamMember.Is_Admin and (CompareText(CurrentUser.AWUserInfo.FLoginName,FTeamMember.Email) =0) then
          begin
        ...
        else if CompareText(CurrentUser.AWUserInfo.FloginName, FTeamMember.Email)  = 0 then //if the person creates the order and also a team member then assign to that person
           begin
...
//          chkShowPending.Checked := FUploadInfo2.IsAdmin;
          chkShowPending.Checked := FTeamNameList.Count > 0;
...
begin
  ...
  case FVersionNumber of
    0,1: result := '1.1';
    2: result := IALVersionID; //github 617 use global variable value
    else result := '';
  end;
end;
...
begin
  ...
  //      nBuild := GetValidInteger(popStr(aTemp,'.'));   //github # 659

        aTemp := cVersionID;
        cMajor := GetValidInteger(popStr(aTemp,'.'));
        cMinor := GetValidInteger(popStr(aTemp,'.'));
  //      cBuild := GetValidInteger(popStr(aTemp,'.'));  //github #659
        ...
        if cMajor < nMajor then
          ...
          begin
             if cMinor < nMinor then
               showWarning := True
  //           else if cBuild < nBuild then   //github #659
  //             showWarning := True;   //github #659
          end;
...
//      GetCityStZip(CityStZip, city, state, zip);  //github #662
        ...
        begin
          if SummaryGrid.Cell[sSelect,i] = cbChecked then
            begin
              ...
              if (pos('C', aStatus) > 0) or (pos('D', aStatus) > 0) then  //this is a completed one
                begin
                  aContinue := True;
                  break;
                end
...
begin
  FUploadInfo2.UserName := CurrentUser.AWUserInfo.FLoginName;
  FUploadInfo2.Password := CurrentUser.AWUserInfo.FPassword;
  SummaryGrid.Rows := 0;
  try
    if chkReadyForImport.Checked then
      begin
...
(*  Don't need this, we just load straight the time field not showing anywhere.
    begin
      try
        if (pos('PM', UpperCase(aUploadInfo.Insp_time)) > 0) or
           (pos('AM', UpperCase(aUploadInfo.Insp_time))> 0) or
           (length(aUploadInfo.insp_time) < 10) then
          insp_time := FormatDateTime('hh:mm ampm',StrTotime(aUploadInfo.Insp_time))
        else
          begin
            aUploadInfo.Insp_time := StringReplace(UpperCase(aUploadInfo.Insp_time), 'T',' ',[rfReplaceAll]);
            aUploadInfo.Insp_time := SetISOtoDateTime(aUploadInfo.insp_time);
            if pos(' ', aUploadInfo.Insp_time) > 0 then
              popStr(aUploadInfo.Insp_time, ' ');
            insp_time := FormatDateTime('hh:mm ampm',StrTotime(aUploadInfo.Insp_time));
          end;
      except on E:Exception do
        insp_time := aUploadInfo.Insp_time;
...
procedure TCC_Mobile_Inspection.SummaryGridClick(Sender: TObject);
var
  curRow: Integer;
begin
  if SummaryGrid.Rows = 0 then exit;
  curRow := SummaryGrid.CurrentDataRow;
  if curRow <= 0 then exit;
  SummaryGrid.RowSelected[SummaryGrid.CurrentDataRow] := True;
end;
...
//  btnReassigned.Enabled := FUploadInfo2.IsAdmin;
  if DataColUp = sSelect then
    begin
      for aRow := 1 to SummaryGrid.Rows do  //reset checked to unchecked
        begin
          if not ControlKeyDown then
            begin
              if SummaryGrid.CellCheckBoxState[sSelect, aRow] = cbChecked then
                SummaryGrid.cell[sSelect, aRow] := 0 //reset
            end;
        end;
      if not ControlKeyDown then
        begin
          if checked then
            SummaryGrid.Cell[sSelect, DataRowUp] := 1
        end;
    end;
