...
interface
  ...
  function MLS_WebServiceList_CRM(UserCredential:TAWCredentials):Boolean;
  function MLS_WebServiceEnhancement_CRM(userID,userCoKey,orderKey : String; SecToken : WideString; MLS_ID : Integer; var MLSResponseData:WideString) : String;
...
Uses
  ...
  UMLS_Globals, UMLS_DataModule, {UWebConfig,} UMLS_ImportData,
  XMLDoc, XMLIntf, UGlobals, UWindowsInfo, uStatus,
  UCRMServices,uLkJSON,Variants;
...
begin
  ...
  try
    try
      ...
      with GetMlsServerPortType(True,Live_awsi_MLS,nil ) do
        begin
...
begin
  ...
  try
    try
      ResponseTxt := GetCRM_MLSList(UserCredential);
      result := length(ResponseTxt) > 0;
      jsList:= TlkJSON.ParseText(ResponseTxt) as TlkJSonList;
      if jsList <> nil then
        begin
          for i:= 0 to jsList.Count -1 do
            begin
              ...
              MLSDataModule.CDS_MLS_Board.FieldByName('id').AsInteger       := js.Field['MlsId'].Value;
              MLSDataModule.CDS_MLS_Board.FieldByName('MLS_Board').AsString := varToStr(js.Field['MlsName'].Value);
              MLSDataModule.CDS_MLS_Board.FieldByName('State').AsString     := varToStr(js.Field['MlsState'].Value);
...
var
  ...
  i,x, Count : Integer;
  ExtraFields : String;
  mlsdetails: TGetMlsEnhancementDetails;
  mlsresponse:TGetMlsEnhancementResponse;
  responsetxt:WideString;
sl:TStringList;
begin
 PushMouseCursor(crHourglass);
 try
  try
    mlsresponse := TGetMlsEnhancementResponse.Create;
    Buffer := TStringList.Create;
    Buffer.LoadFromFile(CC_MLSImport.cbxFiles.Text);

    // MLS Details - Buffer Data and MLS ID
    mlsdetails := TGetMlsEnhancementDetails.Create;
    mlsdetails.MlsId := MLS_ID;
// sl:=TStringList.Create;
// sl.Text := Buffer.Text;
// sl.SaveToFile('c:\temp\B464Encode_Buffer.txt');
    mlsdetails.DataBuffer := Base64Encode(Buffer.Text);
// sl.text := mlsdetails.DataBuffer;
// sl.SaveToFile('c:\temp\After64Encode_Buffer.txt');
// sl.Free;
    GetCRM_MLSData(CurrentUser.AWUserInfo,mlsdetails,ResponseTxt);
    if Length(ResponseTxt) > 0 then
      begin
        mlsResponse.Results.code := 0;
        ResponseTxt := trim(ResponseTxt);
        if CompareText(ResponseTxt, 'No Map') = 0 then
          mlsresponse.Results.Code := 2090100
        else
          mlsResponse.ResponseData.DataBuffer := Base64Decode(ResponseTxt);
      end;
    if mlsresponse.Results.Code = 0 then
      begin
        //mlsresponse.ResponseData.ServiceAcknowledgement
