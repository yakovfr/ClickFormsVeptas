...
type
  TUSPostalService = class(TAdvancedForm)
    ...
    procedure TempWebBrowserStatusTextChange(Sender: TObject;
      const Text: WideString);
    procedure TempWebBrowserNewWindow2(Sender: TObject;
      var ppDisp: IDispatch; var Cancel: WordBool);
    procedure TempWebBrowserPropertyChange(Sender: TObject;
...
//  FFIEC_RESULT_URL  = 'http://www.ffiec.gov/geocode/Geocodesearch.aspx?';
  FFIEC_RESULT_URL  = 'https://geomap.ffiec.gov/FFIECGeocMap/GeocodeMap1.aspx';
...
procedure TUSPostalService.FormCreate(Sender: TObject);
begin
   TempWebBrowser.Silent := True;
end;

procedure TUSPostalService.TempWebBrowserStatusTextChange(Sender: TObject;
  const Text: WideString);
begin
  showmessage('on status text change: '+ text);
end;

procedure TUSPostalService.TempWebBrowserNewWindow2(Sender: TObject;
  var ppDisp: IDispatch; var Cancel: WordBool);
begin
  Cancel := False;
end;

procedure TUSPostalService.TempWebBrowserPropertyChange(Sender: TObject;
  const szProperty: WideString);
begin
  ShowMessage('OnPropertyChange: '+szProperty);
end;
