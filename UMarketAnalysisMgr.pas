...
uses
  ...
  UCell,
  UContainer,
  ULicUser, UAWSI_Utils, AWSI_Server_MarketConditions,
  USubscriptionMgr;
...
const
  ...
  CFormFNMA2055 = 355;
  CFormFNMA1075 = 347;
  CFormNonLenderRes = 736;  //github 320 Non-Lender Residential
  CFormNonLenderCondo = 735;  //github 320 Non-Lender Condo
  CFormNonLenderExtR = 764; //github 320 Non-Lender Exterior Residential
  CFormNonLenderInc = 737; //github 320 Non-Lender Small Residential Income
...
type
  ...
  // mls data
  TMLSRequestDetails = class(clsMonthlyMlsRequestDetails)
    private
      FDecoder: TIdDecoderMime;
      FEncoder: TIdEncoderMime;
    private
    ...
    public
      procedure Assign(Source: clsMonthlyMlsRequestDetails); virtual;
      procedure AssignTo(Dest: clsMonthlyMlsRequestDetails); virtual;
      constructor Create; override;
      destructor Destroy; override;
    published
  ...
  TMarketConditionsReport = class(TPersistent)
    private
      ...
      FResponse: clsMonthlyMarketCondition4ConnectionResponse;
    ...
    private
      ...
      procedure FillForm736(const Container: TContainer); //github 320
      procedure FillForm735(const Container: TContainer); //github 320
      procedure FillForm764(const Container: TContainer); //github 320
      procedure FillForm737(const Container: TContainer); //github 320
    ...
    public
      ...
      procedure GetMLSProviders(OK2UseCRM:Boolean; out Response: clsMlsDirectoryConnectionResponse; const Territory: String);
      procedure GetStates(OK2UseCRM:Boolean; out Response: clsMlsDirectoryStatesConnectionResponse);
    public
      property OK2UseCRM:Boolean read FOK2UseCRM write FOK2UseCRM;
...
var
  MLS: clsMonthlyMlsRequestDetails;
  Credentials : clsUserCredentials;
  ACKResponse: clsAcknowledgement;
  Token,CompanyKey,OrderKey : WideString;
  aAWLogin, aAWPsw, aAWCustUID,VendorTokenKey: String;
  aContinue: Boolean;
begin
  MLS := clsMonthlyMlsRequestDetails.Create;
  aContinue := False;
  try
    CoInitialize(nil);
    {Get Token,CompanyKey and OrderKey}
    if FReport.OK2UseCRM then
      begin
      ...
      begin
        ...
        aContinue := AWSI_GetCFSecurityToken(aAWLogin, aAWPsw,aAWCustUID, Token, CompanyKey, OrderKey, True);
      end;
    if aContinue then
      begin
        try
        ...
        FReport.FResponse := FReport.FService.MarketConditionServices_CreateMonthlyMarketCondition4Report(Credentials, MLS, FReport.FSubject);
        if FReport.FResponse.Results.Code = 0 then
          begin
            try
              //acknowledge response
              ACKResponse := clsAcknowledgement.Create;
              ACKResponse.Received := 1;
              ACKResponse.ServiceAcknowledgement := FReport.FResponse.ResponseData.ServiceAcknowledgement;
              FReport.FService.MarketConditionServices_Acknowledgement(Credentials, ACKResponse);
            finally
              ACKResponse.Free;
            end;
          end;
...
procedure TMLSRequestDetails.Assign(Source: clsMonthlyMlsRequestDetails);
//AWprocedure TMLSRequestDetails.Assign(Source: clsMlsRequestDetails);
begin
...
procedure TMLSRequestDetails.AssignTo(Dest: clsMonthlyMlsRequestDetails);
//AWprocedure TMLSRequestDetails.AssignTo(Dest: clsMlsRequestDetails);
begin
...
begin
  ...
  FService := GetMarketConditionServerPortType(true, TWebPaths.MarketConditionServerWSDL);
...
begin
  ...
  try
    ...
    FillForm736(container); //github 320
    FillForm735(container); //github 320
    FillForm764(container); //github 320
    FillForm737(container); //github 320
...
begin
  ...
  if form = nil then
    form := Container.GetFormByOccurance(CFormFMAC70H, 0, false); //check for FNMA 70H
  if Assigned(form) then
    begin
      // marketing time
      if (StrToIntDef(FResponse.ResponseData.QuarterlyResults.Months1To3.real_doms, 0) < 3 * 30) then        // < 3 months
        (form.frmPage[0].pgData[66] as TChkBoxCell).SetCheckMark(true)
      else if (StrToIntDef(FResponse.ResponseData.QuarterlyResults.Months1To3.real_doms, 0) < 6 * 30) then   // < 6 months
...
begin
  ...
  if Assigned(form) then
    begin
      ...
      form.SetCellText(1, 12, FResponse.ResponseData.QuarterlyResults.Months1To3.sold);
      form.SetCellText(1, 11, FResponse.ResponseData.QuarterlyResults.Months4To6.sold);
      form.SetCellText(1, 10, FResponse.ResponseData.QuarterlyResults.Months7To12.sold);
      form.SetCellText(1, 18, FResponse.ResponseData.QuarterlyResults.Months1To3.absorption_rate);
      form.SetCellText(1, 17, FResponse.ResponseData.QuarterlyResults.Months4To6.absorption_rate);
      form.SetCellText(1, 16, FResponse.ResponseData.QuarterlyResults.Months7To12.absorption_rate);
      form.SetCellText(1, 24, FResponse.ResponseData.QuarterlyResults.Months1To3.active);
      form.SetCellText(1, 23, FResponse.ResponseData.QuarterlyResults.Months4To6.active);
      form.SetCellText(1, 22, FResponse.ResponseData.QuarterlyResults.Months7To12.active);
      form.SetCellText(1, 30, FResponse.ResponseData.QuarterlyResults.Months1To3.months_of_supply);
      form.SetCellText(1, 29, FResponse.ResponseData.QuarterlyResults.Months4To6.months_of_supply);
      form.SetCellText(1, 28, FResponse.ResponseData.QuarterlyResults.Months7To12.months_of_supply);
//AW
...
//AW
      ...
      form.SetCellText(1, 36, Trim(Format(ValFmt, [StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months1To3.sale_prices, 0)])));
      form.SetCellText(1, 35, Trim(Format(ValFmt, [StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months4To6.sale_prices, 0)])));
      form.SetCellText(1, 34, Trim(Format(ValFmt, [StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months7To12.sale_prices, 0)])));
      form.SetCellText(1, 42, IntToStr(UUtil1.Round(StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months1To3.real_doms, 0))));
      form.SetCellText(1, 41, IntToStr(UUtil1.Round(StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months4To6.real_doms, 0))));
      form.SetCellText(1, 40, IntToStr(UUtil1.Round(StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months7To12.real_doms, 0))));
      form.SetCellText(1, 48, Trim(Format(ValFmt, [StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months1To3.list_prices, 0)])));
      form.SetCellText(1, 47, Trim(Format(ValFmt, [StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months4To6.list_prices, 0)])));
      form.SetCellText(1, 46, Trim(Format(ValFmt, [StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months7To12.list_prices, 0)])));
      form.SetCellText(1, 54, IntToStr(UUtil1.Round(StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months1To3.listing_days, 0))));
      form.SetCellText(1, 53, IntToStr(UUtil1.Round(StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months4To6.listing_days, 0))));
      form.SetCellText(1, 52, IntToStr(UUtil1.Round(StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months7To12.listing_days, 0))));
      form.SetCellText(1, 60, IntToStr(UUtil1.Round(StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months1To3.ratios, 0) * 100)) + '%');
      form.SetCellText(1, 59, IntToStr(UUtil1.Round(StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months4To6.ratios, 0) * 100)) + '%');
      form.SetCellText(1, 58, IntToStr(UUtil1.Round(StrToFloatDef(FResponse.ResponseData.QuarterlyResults.Months7To12.ratios, 0) * 100)) + '%');
...
//github 320  Non-Lender Residential form
procedure TMarketConditionsReport.FillForm736(const Container: TContainer);
var
  form: TDocForm;
begin
  form := Container.GetFormByOccurance(CFormNonLenderRes, 0, false);
  if Assigned(form) then
    begin
      // marketing time
      if (StrToIntDef(FResponse.ResponseData.QuarterlyResults.Months1To3.real_doms, 0) < 3 * 30) then        // < 3 months
        (form.frmPage[0].pgData[63] as TChkBoxCell).SetCheckMark(true)
      else if (StrToIntDef(FResponse.ResponseData.QuarterlyResults.Months1To3.real_doms, 0) < 6 * 30) then   // < 6 months
        (form.frmPage[0].pgData[64] as TChkBoxCell).SetCheckMark(true)
      else                                                                                            // 6+ months
        (form.frmPage[0].pgData[65] as TChkBoxCell).SetCheckMark(true);

      // market conditions
      if (form.GetCellText(1, 81) = '') then
        form.SetCellText(1, 81, FResponse.ResponseData.UrarResults.Extra_Comments.urar_comments);
...
//github 320 Non Lender Condo Appraisal
procedure TMarketConditionsReport.FillForm735(const Container: TContainer);
var
  form: TDocForm;
begin
  form := Container.GetFormByOccurance(CFormNonLenderCondo, 0, false);
  if Assigned(form) then
    begin
      // marketing time
      if (StrToIntDef(FResponse.ResponseData.QuarterlyResults.Months1To3.real_doms, 0) < 3 * 30) then        // < 3 months
      ...
      // market conditions
      if (form.GetCellText(1, 82) = '') then
        form.SetCellText(1, 82, FResponse.ResponseData.UrarResults.Extra_Comments.urar_comments);
...
//github 320 non lender exterior residential appraisal
procedure TMarketConditionsReport.FillForm764(const Container: TContainer);
var
  form: TDocForm;
begin
  form := Container.GetFormByOccurance(CFormNonLenderExtR, 0, false);
  if Assigned(form) then
    begin
      // marketing time
      if (StrToIntDef(FResponse.ResponseData.QuarterlyResults.Months1To3.real_doms, 0) < 3 * 30) then        // < 3 months
...
//github 320 non lender small residential income property
procedure TMarketConditionsReport.FillForm737(const Container: TContainer);
var
  form: TDocForm;
begin
  form := Container.GetFormByOccurance(CFormNonLenderInc, 0, false);
  if Assigned(form) then
    begin
      // marketing time
      if (StrToIntDef(FResponse.ResponseData.QuarterlyResults.Months1To3.real_doms, 0) < 3 * 30) then        // < 3 months
...
procedure TMarketConditionsReport.GetMLSProviders(OK2UseCRM:Boolean; out response: clsMlsDirectoryConnectionResponse; const territory: string);
var
  ...
  try
    ...
    if OK2UseCRM then
      begin
        aAWLogin := CurrentUser.AWUserInfo.UserLoginEmail;
        aAWPsw   := CurrentUser.AWUserInfo.UserPassWord;
        aAWCustUID := CF_Trial_CustID;
      end
    else
      CurrentUser.GetUserAWLoginByLicType(pidOnlineLocMaps, aAWLogin, aAWPsw, aAWCustUID); //base on the license type to return either the Real user or trial user
    if AWSI_GetCFSecurityToken(aAWLogin, aAWPsw, aAWCustUID, Token, CompanyKey, OrderKey, True) then
      begin
        {User Credentials}
        Credentials := clsUserCredentials.create;
        Credentials.Username := aAWLogin;
        ...
        Credentials.OrderNumberKey := OrderKey;
        Credentials.Purchase := 0;
        response := FService.MarketConditionServices_GetMlsDirectoryListing(Credentials, mls);
      end;
  finally
...
procedure TMarketConditionsReport.GetStates(OK2UseCRM:Boolean; out Response: clsMlsDirectoryStatesConnectionResponse);
var
  Credentials : clsUserCredentials;
  Token,CompanyKey,OrderKey : WideString;
  aAWLogin, aAWPsw, aAWCustUID: String;
  ...
  try
    ...
    if AWSI_GetCFSecurityToken(aAWLogin, aAWPsw,aAWCustUID, Token, CompanyKey, OrderKey, True) then
      begin
        ...
        Credentials.OrderNumberKey := OrderKey;
        Credentials.Purchase := 0;
        response := FService.MarketConditionServices_GetMlsDirectoryStatesListing(Credentials);
      end;
  finally
    FreeAndNil(Credentials);
  end;
