...
{*******************************************************}
{    The memory handling in the delphi code had a bug   }
{    so we copied the code and worked around the bug.   }
{ The bug has to do with TotalAllocatedMen and          }
{ AllocMemSize which can be wrong in a lot of instances.}
{*******************************************************}
unit UMxArrays;
...
resourcestring
  ...
  sLowCapacityError     = 'The Memory Capacity is low. Failure may occur soon.';
...
var
  A_TotalAllocatedMemory: Integer;  { Total allaocted by the array classes }
...
begin
  MemStats.dwLength := SizeOf(TMemoryStatus);
...
begin
  Result := False;
  Dec(A_TotalAllocatedMemory, oldSize);
...
begin
  ...
  begin
    try
      Dec(A_TotalAllocatedMemory, FMemAllocated);              //dec the global memory meter
...
begin
 ...
 { if ((FMemAllocated + A_TotalAllocatedMemory) > AvailableMemory) then
    begin
//out      ShowNotice('Err2: MemAllocated= '+ IntToStr(FMemAllocated)+ ', TotalAllocated= '+IntToStr(A_TotalAllocatedMemory)+', AvailableMemory= '+IntToStr(AvailableMemory));
      LowCapacityError;
    end;                  }     //EOutOfMemory will be raised if not enough memory
  ...
  Inc(A_TotalAllocatedMemory, FMemAllocated);
end;
...
begin
  ...
  {if ((FMemAllocated + A_TotalAllocatedMemory) > AvailableMemory) then
    LowCapacityError;    }   //EOutOfMemory will be raised if not enough memory
...
begin
  ...
  begin
    //try
      ...
      if idxElements = nil then
        raise ELowCapacityError.CreateRes(@sLowCapacityError);
      Inc(A_TotalAllocatedMemory, NewCapacity*sizeof(TSmallIntArray));
    {except
      FreeMem(idxElements);
      raise;
    end;    }
