...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, ExtCtrls, RzLabel, Buttons,
  ULicUser, UForms, ULicConfirmation, Grids;
...
type
  TRegistration2 = class(TAdvancedForm)
    Panel1: TPanel;
    btnCancel: TButton;
    btnPrev: TButton;
    btnNext: TButton;
    ...
    tabCompanyOwn: TTabSheet;
    rdoIndividual: TRadioButton;
    rdoAddedUser: TRadioButton;
  ...
  private
    ...
    FLicMode: Integer;
    FUserLicInfo: TUserLicInfo;                   //used to pass data between UserLic and AW
    FModified: Boolean;
    ...
    function ValidatedAddedUserInfo: Boolean;
    function ValidateAppraisalCerts: Boolean;
    function ValidatedReportSigner: Boolean;
    procedure SetupIntroPage;
    procedure LoadToDisplayFrom(UserInfo: TUserLicInfo);        //displays it
    procedure SaveFromDisplayTo(UserInfo: TUserLicInfo);        //captures user input
    procedure SaveRegistrationToLicenseFile;
    procedure ConfirmEvaluationSoftwareLicense(UserInfo: TUserLicInfo);
    procedure ConfirmPaidSoftwareLicense(UserInfo: TUserLicInfo);
    procedure ConfirmAppraisalWorldMembership(UserInfo: TUserLicInfo;LoadLicInfo:Boolean=True);
    function ValidateUserSelected: Boolean;
    procedure LoadAllLicensedUsers;
    procedure LoadLicensedUser(User: TLicensedUser);
  public
    constructor CreateRegistar(AOWner: TComponent; AUser: TLicensedUser; LicMode: Integer; ShowUserSelection:Boolean);
    destructor Destroy;   override;

    procedure RegisterSoftwareUsage;
    property AWMemberID: String read FAWMemberID write FAWMemberID;
  end;



  function RegisterClickFORMSSoftware(AOwner: TComponent; User: TLicensedUser; LicMode: Integer; ShowUserSelection:Boolean=True): Boolean;
...
begin
  try
    ...
    UserReg := TRegistration2.CreateRegistar(nil, User, LicMode, ShowUserSelection);
...
constructor TRegistration2.CreateRegistar(AOWner: TComponent; AUser: TLicensedUser; LicMode: Integer; ShowUserSelection:Boolean);
begin
  ...
  FUserLicInfo := TUserLicInfo.Create;
...
destructor TRegistration2.Destroy;
begin
...
procedure TRegistration2.LoadAllLicensedUsers;
var
  i: Integer;
begin
...
procedure TRegistration2.SetupIntroPage;
begin
...
procedure TRegistration2.tabAWLoginShow(Sender: TObject);
begin
...
procedure TRegistration2.SaveFromDisplayTo(UserInfo: TUserLicInfo);
begin
  if assigned(UserInfo) then
    ...
    //save the users signature name and company name
      ...
      //Added User or Individual?
      UserInfo.FRegAsIndividual := True;
      if rdoAddedUser.Checked then //is the company name locked
        UserInfo.FRegAsIndividual := False;
      if UserInfo.FRegAsIndividual then
        UserInfo.FGroupID := 0;
    end; //with UserInfo
end;

//loads the display objects from data in UserInfo
procedure TRegistration2.LoadToDisplayFrom(UserInfo: TUserLicInfo);
begin
//  if false then
  if assigned(UserInfo) then
    begin
...
procedure TRegistration2.btnPrevClick(Sender: TObject);
var
...
procedure TRegistration2.btnNextClick(Sender: TObject);
var
...
begin
  ...
  else if CurPage = tabCompanyOwn then
    canMove := ValidatedAddedUserInfo
...
//clear any messages
procedure TRegistration2.tabRegisterShow(Sender: TObject);
begin
  if FLicMode = rtPaidLicMode then
...
procedure TRegistration2.btnSignInClick(Sender: TObject);
begin
  ConfirmAppraisalWorldMembership(FUserLicInfo, FLoadLicInfo);
  //lock company name if the locking field is set to True
  edtLicCoName.ReadOnly := FUserLicInfo.FLicCoLocked;
end;

procedure TRegistration2.btnRegisterClick(Sender: TObject);
begin
  RegisterSoftwareUsage;
end;

procedure TRegistration2.btnCancelClick(Sender: TObject);
begin
  Close;
end;

procedure TRegistration2.btnSaveClick(Sender: TObject);
begin
  CRMToken.CRM_Authentication_Token := '';   //need to reset the token each time we bring up clickFORMS
...
procedure TRegistration2.SaveRegistrationToLicenseFile;
begin
...
//  FCurUser.LicInfo.FRegAsIndividual :=
...
procedure TRegistration2.ConfirmAppraisalWorldMembership(UserInfo: TUserLicInfo; LoadLicInfo:Boolean=True);
var
  ...
  if ValidateLoginInfo then
    begin
      ...
      if FUseCRMRegistration then
        ...
        DoConfirmAppraisalWorldMembership(UserEmail, UserPassword, UserInfo, LoadLicInfo);
...
procedure TRegistration2.RegisterSoftwareUsage;
begin
  SaveFromDisplayTo(FUserLicInfo);             //gather the data

  if FLicMode = rtPaidLicMode then
    ConfirmPaidSoftwareLicense(FUserLicInfo)

  else if FLicMode = rtEvalLicMode then
    ConfirmEvaluationSoftwareLicense(FUserLicInfo);
end;

procedure TRegistration2.ConfirmPaidSoftwareLicense(UserInfo: TUserLicInfo);
var
  UserAWUID: String;
begin
  ...
  if FUseCRMRegistration then
    ...
    DoConfirmPaidSoftwareLicense(UserAWUID, UserInfo);
...
procedure TRegistration2.ConfirmEvaluationSoftwareLicense(UserInfo: TUserLicInfo);
var
  UserAWUID: String;
begin
  if FUseCRMRegistration then
    DoConfirmEvaluationSoftwareLicense_CRM(UserInfo)
  else
    DoConfirmEvaluationSoftwareLicense(UserInfo);
...
function TRegistration2.NoData(Str: String): Boolean;
begin
  result := length(Str)=0;
end;

function TRegistration2.ValidateLoginInfo: Boolean;
begin
...
function TRegistration2.ValidateIsAWMember: Boolean;
begin
  result := FUserLicInfo.AWUID > 0;   //the user has a UID, but may not be vaild
end;

function TRegistration2.ValidatedContactInfo: Boolean;
begin
  result := True;
...
function TRegistration2.ValidatedAddedUserInfo: Boolean;
begin
  result := True;
  if not rdoAddedUser.checked and not rdoIndividual.Checked then
    begin
      ShowAlert(atWarnAlert, 'Please select individual or added user.');
      result := False;
    end;
end;

function TRegistration2.ValidateAppraisalCerts: Boolean;
begin
...
function TRegistration2.ValidatedReportSigner: Boolean;
begin
  result := True;
...
function TRegistration2.ValidateUserSelected: Boolean;
var
  userFileName: String;
begin
  result := True;
...
procedure TRegistration2.LoadLicensedUser(User: TLicensedUser);
var
  i: Integer;
begin
