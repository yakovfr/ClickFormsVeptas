...
uses
  ...
  ComCtrls, UContainer,UMLS_DataModule, {UCC_Utils,}
  UGlobals, Grids_ts, TSGrid, TSDBGrid, UMyClickForms,UMLS_Dialog,
...
implementation
  uses
    UStatus, UUtil1,
    UMLS_Reader,UMLS_DBRead, UMLS_Globals, AWSI_Server_MLS,
//    UCC_Globals,
    UUtil2, Math, ULicUser,AWSI_Server_Access;
...
begin
  ...
  try
    try
      with GetAwsiAccessServerPortType(false, AWAccess_LIVE_URL) do
      ...
      try
        try
          ...
          if FUseCRMRegistration then
            begin
              if MLS_WebServiceList_CRM(CurrentUser.AWUserInfo) then
            ...
            begin
              CurrentUser.GetUserAWLoginByLicType(pidMLSDataImport, aAWLogin, aAWPsw, aAWCustUID); //base on the license type to return either the Real user or trial user
              if AWSI_GetCFSecurityToken(aAWLogin,aAWPsw, aAWCustUID, FSecurityToken, FCompanyKey, FOrderKey, True) then
                begin
                  //Highlight the state in state list form preferences
                  lbxStateList.ItemIndex := lbxStateList.Items.IndexOfName(Import_MLSName);
                  if not AWSI_GetSecurityTokenEx(aAWLogin,aAWPsw, FCompanyKey, FOrderKey, FSecurityToken) then
                    begin
                    ...
                    begin
                      if not MLS_WebServiceList(aAWLogin,FCompanyKey, FOrderKey, FSecurityToken) then
                        //else if not MLS_WebServiceList(CurrentUser.AWUserInfo.FLoginName, CurrentUser.AWUserInfo.FPassword,'Appraisal.Order.BillTo.CompanyKey', 'Appraisal.Order.BillTo.AWOrderKey', secToken) then
                        begin
    ...
    Application.ProcessMessages;
         ...
         if (Import_MLSName = '') or (Import_MLSFile = '') then
           ...
           //start the processing
             ...
             if FUseCRMRegistration then //###TODO
               begin
                 resp := MLS_WebServiceEnhancement_CRM(aAWLogin,FCompanyKey,FOrderKey, FSecurityToken, Import_MLSID, MLSResponseData);
               end
             else
               begin
                 CurrentUser.GetUserAWLoginByLicType(pidMLSDataImport, aAWLogin, aAWPsw, aAWCustUID); //base on the license type to return either the Real user or trial user
                 resp := MLS_WebServiceEnhancement(aAWLogin,FCompanyKey,FOrderKey, FSecurityToken, Import_MLSID, MLSResponseData);
               end;
             if resp = '' then
               resp := 'Success';  //PAM
