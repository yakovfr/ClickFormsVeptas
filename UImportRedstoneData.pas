                       ...
                       if cellValue <> '' then
                         begin
                           ...
                           if aCell.FMathID > 0 then
                            if aCell is TChkBoxCell then
                              aCell.ProcessMath //check boxes do not have PostProcess function but do have ProcessMath; we need to do math for drive storage calculatiobs
                            else
                             aCell.PostProcess;
                         end;
                     end
            ...
            begin
              if (cellID =CGeocodedGridCellID*10) and (CompCol.GetCellByID(CGeocodedGridCellID) is TGeocodedGridCell) then
                begin
                  ...
                  begin
                    if (compNo = 0) and (cellID=1020) and (getValidInteger(str) = 0) then  //ticket #1420 if subject grid fireplace = 0 donot set value
                      continue
                    else if CellID <> 964 then
            ...
            begin
              if (cellID =CGeocodedGridCellID*10) and (CompCol.GetCellByID(CGeocodedGridCellID) is TGeocodedGridCell) then
                begin
                  ...
                  begin
                    if CellID <> 964 then  //skip cell value 964
                      SetCompTableValue(doc, UADObject, CompCol, cellID, str);
                  end;
                break;
            end;
                ...
                if compNo = i then
                  begin
                    if (cellID =CGeocodedGridCellID * 10) and (CompCol.GetCellByID(CGeocodedGridCellID) is TGeocodedGridCell) then
                      ...
                      else if (str <> '') and (CellID <> 964) then
                        SetCompTableValue(doc, UADObject, CompCol, cellID, str);
                      break;
                  end;
              end;
...
begin
  ...
  for f := 0 to doc.docForm.Count - 1 do
    begin
      ...
      case frmID of
        340, 4218, 4365, 349, 345, 347, 355:  //4218 added 1004P, 4365 - FMAC 70H
              ...
              begin
                ...
                if pos('L-', cellStr) > 0 then //this is listing
                  ...
                  begin
                    ...
                    if CellID <> 964 then  //#1646: skip feeshimple
